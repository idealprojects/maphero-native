if (MH_LEGACY_RENDERER)
  message(FATAL "The legacy renderer is no longer supported")
endif()

if (MH_DRAWABLE_RENDERER)
  message(FATAL "Do not pass MH_DRAWABLE_RENDERER, the drawable renderer is now the default")
endif()

set(backend_count 0)
if (MH_WITH_OPENGL)
  math(EXPR backend_count "${backend_count} + 1")
endif()
if (MH_WITH_METAL)
  math(EXPR backend_count "${backend_count} + 1")
endif()
if (MH_WITH_VULKAN)
  math(EXPR backend_count "${backend_count} + 1")
endif()
if (MH_WITH_WEBGPU)
  math(EXPR backend_count "${backend_count} + 1")
endif()

if (backend_count EQUAL 0)
  message(FATAL_ERROR
    "You need to set a rendering backend. "
    "Set exactly one of: MH_WITH_OPENGL, MH_WITH_METAL, MH_WITH_VULKAN, or MH_WITH_WEBGPU.")
elseif (backend_count GREATER 1)
  message(FATAL_ERROR
    "Multiple rendering backends selected. "
    "Please enable only one of: MH_WITH_OPENGL, MH_WITH_METAL, MH_WITH_VULKAN, or MH_WITH_WEBGPU.")
endif()
