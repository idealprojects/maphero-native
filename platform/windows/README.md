...
```cmd
cmake . -B build -G Ninja -DCMAKE_BUILD_TYPE=Release -DMH_WITH_EGL=ON
```

To configure build with OSMesa (software rendering), use the following command:

```
cmake . -B build -G Ninja -DCMAKE_BUILD_TYPE=Release -DMH_WITH_OSMESA=ON
```
...
```cmd
cmake . -B build -DMH_WITH_EGL=ON
```
or
```cmd
cmake . -B build -DMH_WITH_OSMESA=ON
```
