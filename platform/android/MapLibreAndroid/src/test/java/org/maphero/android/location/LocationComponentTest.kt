...
import org.maphero.android.location.LocationComponentConstants.TRANSITION_ANIMATION_DURATION_MS
...
class LocationComponentTest {
    ...
    private lateinit var developerAnimationListeners: List<MapHeroMap.OnDeveloperAnimationListener>
    ...
    fun before() {
        ...
        locationComponent = LocationComponent(mapHeroMap, transform, developerAnimationListeners, currentListener, lastListener, locationLayerController, locationCameraController, locationAnimatorCoordinator, staleStateManager, compassEngine, false)
        doReturn(style).`when`(mapHeroMap).style
    ...
    fun transitionCallbackFinishedTest() {
        ...
        locationComponent.onStart()
        locationComponent.isLocationComponentEnabled = true
        `when`(mapHeroMap.cameraPosition).thenReturn(CameraPosition.DEFAULT)
    ...
    fun change_to_gps_mode_symbolLayerBearingValue() {
        ...
        `when`(mapHeroMap.projection).thenReturn(projection)
        `when`(mapHeroMap.cameraPosition).thenReturn(CameraPosition.DEFAULT)
    ...
    fun tiltWhileTracking_notReady() {
        ...
        val callback = mock(MapHeroMap.CancelableCallback::class.java)
    ...
    fun newLocation_accuracy_indicatorLayerRadiusValue() {
        val location = Location("test")
        location.accuracy = 50f
        locationComponent = LocationComponent(mapHeroMap, transform, developerAnimationListeners, currentListener, lastListener, locationLayerController, locationCameraController, locationAnimatorCoordinator, staleStateManager, compassEngine, true)
        locationComponent.activateLocationComponent(
            LocationComponentActivationOptions.builder(context, style)
                .locationComponentOptions(locationComponentOptions)
