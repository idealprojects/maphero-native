...
import org.maphero.android.maps.IconManagerResolver;
...
public class IconTest extends EspressoTest {
  ...
  public void testAddDifferentIconMarker() {
    validateTestSetup();
    Icon icon = IconFactory.getInstance(rule.getActivity()).fromResource(R.drawable.maphero_compass_icon);
  ...
  public void testAddRemoveMany() {
    ...
    maplibreMap.addMarker(new MarkerOptions()
      .icon(iconFactory.fromResource(R.drawable.maphero_mylocation_icon_default))
      .position(new LatLng(3, 1))
    );
    maplibreMap.addMarker(new MarkerOptions()
      .icon(iconFactory.fromResource(R.drawable.maphero_compass_icon))
