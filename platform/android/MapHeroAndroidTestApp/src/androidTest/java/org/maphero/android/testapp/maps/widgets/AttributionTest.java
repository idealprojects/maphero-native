package org.maphero.android.testapp.maps.widgets;
...
public class AttributionTest extends EspressoTest {
  ...
  public void testMapboxStreetsMapboxAttributionLink() {
    ...
    onView(withText(R.string.MAPHERO_attributionsDialogTitle)).check(matches(isDisplayed()));
  ...
  private void buildUrlSpans() {
    onView(withId(R.id.mapView)).perform(new MapHeroMapAction((uiController, view) -> {
  ...
  private class MapHeroMapAction implements ViewAction {

    private InvokeViewAction invokeViewAction;

    MapHeroMapAction(InvokeViewAction invokeViewAction) {
