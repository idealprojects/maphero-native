...
class DynamicInfoWindowAdapterActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    private fun addMarker(maplibreMap: MapHeroMap): Marker {
            ...
            MarkerOptions()
                ...
                .icon(
                    IconUtils.drawableToIcon(
                        ...
                        ResourcesCompat.getColor(resources, R.color.MAPHERO_blue, theme)
                    )
                )
        )
    }

    private fun addCustomInfoWindowAdapter(maplibreMap: MapHeroMap) {
