...
class TextureViewTransparentBackgroundActivity : AppCompatActivity() {
    ...
    private val maplibreMap: MapHeroMap? = null
    ...
    private fun setupMapView(savedInstanceState: Bundle?) {
        val maplibreMapOptions = MapHeroMapOptions.createFromAttributes(this, null)
        ...
        mapView.getMapAsync { maplibreMap: MapHeroMap -> initMap(maplibreMap) }
        (findViewById<View>(R.id.coordinator_layout) as ViewGroup).addView(mapView)
    }

    private fun initMap(maplibreMap: MapHeroMap) {
        try {
