...
class MapHeroMapOptionsAttrsTest {
    ...
    private fun mockEnableLocalIdeograph(enabled: Boolean) {
        every {
            typedArray.getBoolean(R.styleable.maphero_MapView_maplibre_localIdeographEnabled, true)
        }.returns(enabled)
    }

    private fun mockLocalIdeographString(font: String) {
        every {
            typedArray.getString(R.styleable.maphero_MapView_maplibre_localIdeographFontFamily)
        }.returns(font)
    }
    ...
    private fun mockLocalIdeographStringArray(fonts: Array<String>) {
        ...
        every {
            typedArray.getResourceId(R.styleable.maphero_MapView_maplibre_localIdeographFontFamilies, 0)
        }.returns(resId)
