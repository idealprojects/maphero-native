...
class CameraPositionTest : BaseTest() {
    ...
    fun testTypedArrayBuilder() {
        ...
        Mockito.`when`(
            typedArray.getFloat(
                R.styleable.MAPHERO_MapView_MAPHERO_cameraBearing,
                0.0f
            )
        ).thenReturn(bearing)
        Mockito.`when`(
            typedArray.getFloat(
                R.styleable.MAPHERO_MapView_MAPHERO_cameraTargetLat,
                0.0f
            )
        ).thenReturn(latitude)
        Mockito.`when`(
            typedArray.getFloat(
                R.styleable.MAPHERO_MapView_MAPHERO_cameraTargetLng,
                0.0f
            )
        ).thenReturn(longitude)
        Mockito.`when`(typedArray.getFloat(R.styleable.MAPHERO_MapView_MAPHERO_cameraZoom, 0.0f))
            .thenReturn(zoom)
        Mockito.`when`(typedArray.getFloat(R.styleable.MAPHERO_MapView_MAPHERO_cameraTilt, 0.0f))
            .thenReturn(tilt)
