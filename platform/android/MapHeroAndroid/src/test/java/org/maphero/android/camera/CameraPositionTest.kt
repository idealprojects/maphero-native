...
import org.maphero.android.camera.CameraUpdateFactory.zoomTo
...
class CameraPositionTest {
    ...
    fun testTypedArrayBuilder() {
        ...
        Mockito.`when`(
            typedArray.getFloat(
                R.styleable.maphero_MapView_maplibre_cameraBearing,
                0.0f
            )
        ).thenReturn(bearing)
        Mockito.`when`(
            typedArray.getFloat(
                R.styleable.maphero_MapView_maplibre_cameraTargetLat,
                0.0f
            )
        ).thenReturn(latitude)
        Mockito.`when`(
            typedArray.getFloat(
                R.styleable.maphero_MapView_maplibre_cameraTargetLng,
                0.0f
            )
        ).thenReturn(longitude)
        Mockito.`when`(typedArray.getFloat(R.styleable.maphero_MapView_maplibre_cameraZoom, 0.0f))
            .thenReturn(zoom)
        Mockito.`when`(typedArray.getFloat(R.styleable.maphero_MapView_maplibre_cameraTilt, 0.0f))
            .thenReturn(tilt)
