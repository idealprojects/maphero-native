    ...
    fun setUp() {
        Mockito.`when`(
            context!!.obtainStyledAttributes(
                R.style.MAPHERO_LocationComponent,
                R.styleable.MAPHERO_LocationComponent
            )
        )
            .thenReturn(array)
        Mockito.`when`(
            array!!.getResourceId(
                R.styleable.MAPHERO_LocationComponent_MAPHERO_foregroundDrawable,
                -1
            )
        )
            .thenReturn(R.drawable.maphero_user_icon)
