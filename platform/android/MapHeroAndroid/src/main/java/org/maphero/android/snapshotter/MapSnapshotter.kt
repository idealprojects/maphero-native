...
import org.maphero.android.maps.Style.Builder.*
...
open class MapSnapshotter(context: Context, options: Options) {
    ...
    class Options(width: Int, height: Int) {
         ...
         * Default font for local ideograph font family is [MapHeroConstants.DEFAULT_FONT].
         */
        var localIdeographFontFamily = MapHeroConstants.DEFAULT_FONT
            private set
        ...
        /**
         * @param showLogo The flag indicating to show the MapHero logo.
         * @return the mutated [Options]
         */
    ...
    private fun createTextView(mapSnapshot: MapSnapshot, shortText: Boolean, scale: Float): TextView {
        val textColor = ResourcesCompat.getColor(context.resources, R.color.maphero_gray_dark, context.theme)
    ...
    private fun createScaledLogo(snapshot: Bitmap): Logo {
        val logo = BitmapFactory.decodeResource(context.resources, R.drawable.maphero_logo_icon, null)
        val scale = calculateLogoScale(snapshot, logo)
        val matrix = Matrix()
        matrix.postScale(scale, scale)
        val helmet = BitmapFactory.decodeResource(context.resources, R.drawable.maphero_logo_helmet, null)
