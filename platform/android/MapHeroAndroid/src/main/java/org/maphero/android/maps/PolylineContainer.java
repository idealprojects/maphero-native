...
class PolylineContainer implements Polylines {
  ...
  @Override
  public Polyline addBy(@NonNull PolylineOptions polylineOptions, @NonNull MapHeroMap maplibreMap) {
    Polyline polyline = polylineOptions.getPolyline();
    long id = nativeMap != null ? nativeMap.addPolyline(polyline) : 0;
    polyline.setMapHeroMap(maplibreMap);
    polyline.setId(id);
  ...
  @NonNull
  @Override
  public List<Polyline> addBy(@NonNull List<PolylineOptions> polylineOptionsList, @NonNull MapHeroMap maplibreMap) {
    ...
    if (nativeMap != null && count > 0) {
      ...
      for (int i = 0; i < ids.length; i++) {
        ...
        polylineCreated.setMapHeroMap(maplibreMap);
