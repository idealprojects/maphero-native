...
class IconManager {
  ...
  void ensureIconLoaded(@NonNull Marker marker, @NonNull MapHeroMap maplibreMap) {
    Icon icon = marker.getIcon();
    if (icon == null) {
      icon = loadDefaultIconForMarker(marker);
    }
  ...
  private void setTopOffsetPixels(Marker marker, @NonNull MapHeroMap maplibreMap, @NonNull Icon icon) {
