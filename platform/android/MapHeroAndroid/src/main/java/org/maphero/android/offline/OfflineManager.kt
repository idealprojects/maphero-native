...
import org.maphero.android.geometry.LatLngBounds.Companion.world
...
class OfflineManager private constructor(context: Context) {
     ...
     *
     * Forces MapHero Native to revalidate resources stored in the ambient
    ...
    fun createOfflineRegion(definition: OfflineRegionDefinition, metadata: ByteArray, callback: CreateOfflineRegionCallback) {
        if (!isValidOfflineRegionDefinition(definition)) {
            callback.onError(String.format(context.getString(R.string.MAPHERO_offline_error_region_definition_invalid), definition.bounds))
            return
        }
