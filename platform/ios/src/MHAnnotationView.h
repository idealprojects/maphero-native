...
typedef NS_ENUM(NSUInteger, MHAnnotationViewDragState) {
  /**
   The view is not involved in a drag operation.
   */
  MHAnnotationViewDragStateNone = 0,
  /**
   ...
   */
  MHAnnotationViewDragStateEnding,
};

/**
 The ``MHAnnotationView`` class is responsible for marking a point annotation
 with a view. Annotation views represent an annotation object, which is an
 object that corresponds to the ``MHAnnotation`` protocol. When an annotation’s
 ...
 interactivity such as dragging, you can use an ``MHAnnotationImage`` instead to
 conserve memory and optimize drawing performance.
 */
MH_EXPORT
@interface MHAnnotationView : UIView <NSSecureCoding>
...
/**
 ...
 ``MHMapView/dequeueReusableAnnotationViewWithIdentifier:`` method.
...
/**
 ...
 with the new view and, in custom subclasses of ``MHAnnotationView``, customize the view
 ...
 ``MHMapViewDelegate/mapView:viewForAnnotation:`` method.
 ...
 @return The initialized annotation view object.
 */
- (instancetype)initWithAnnotation:(nullable id<MHAnnotation>)annotation
 ...
 */
@property (nonatomic, nullable) id<MHAnnotation> annotation;
...
/**
 ...
 viewpoint). This is also the behavior of ``MHAnnotationImage`` objects. When the
...
/**
 ...
 view rotates. This is also the behavior of ``MHAnnotationImage`` objects. When the
...
/**
 ...
 ``MHAnnotationView`` or ``MHAnnotationImage`` object, deselects any currently
 selected view.
 ...
 ``MHAnnotationView/setSelected:animated:`` method instead.
 */
...
/**
 ...
 #### Related examples
 TODO: Enable users to drag ``MHAnnotationView`` objects on your map.
 */
...
/**
 ...
 All states are handled automatically when the ``MHAnnotationView/draggable`` property is set to
 `YES`. To perform a custom animation in response to a change to this property,
 override the ``MHAnnotationView/setDragState:animated:`` method.
 */
@property (nonatomic, readonly) MHAnnotationViewDragState dragState;
