...
static const CGFloat MHFeetPerMile = 5280;
...
static const MHRow MHMetricTable[] = {
...
static const MHRow MHImperialTable[] ={
    {.distance = 4, .numberOfBars = 2},
    ...
    {.distance = 1000, .numberOfBars = 2},
    {.distance = 0.25f*MHFeetPerMile, .numberOfBars = 2},
    {.distance = 0.5f*MHFeetPerMile, .numberOfBars = 2},
    {.distance = 1*MHFeetPerMile, .numberOfBars = 2},
    {.distance = 2*MHFeetPerMile, .numberOfBars = 2},
    {.distance = 3*MHFeetPerMile, .numberOfBars = 3},
    {.distance = 4*MHFeetPerMile, .numberOfBars = 2},
    {.distance = 8*MHFeetPerMile, .numberOfBars = 2},
    {.distance = 12*MHFeetPerMile, .numberOfBars = 2},
    {.distance = 15*MHFeetPerMile, .numberOfBars = 3},
    {.distance = 20*MHFeetPerMile, .numberOfBars = 2},
    {.distance = 30*MHFeetPerMile, .numberOfBars = 3},
    {.distance = 40*MHFeetPerMile, .numberOfBars = 2},
    {.distance = 80*MHFeetPerMile, .numberOfBars = 2},
    {.distance = 120*MHFeetPerMile, .numberOfBars = 2},
    {.distance = 200*MHFeetPerMile, .numberOfBars = 2},
    {.distance = 300*MHFeetPerMile, .numberOfBars = 3},
    {.distance = 400*MHFeetPerMile, .numberOfBars = 2},
};

@class MHScaleBarLabel;

@interface MHScaleBar()
@property (nonatomic) NSArray<UIView *> *labelViews;
@property (nonatomic) NSArray<UIView *> *bars;
@property (nonatomic) UIView *containerView;
@property (nonatomic) MHDistanceFormatter *formatter;
@property (nonatomic, assign) MHRow row;
@property (nonatomic, assign) CGFloat borderWidth;
@property (nonatomic) NSMutableDictionary* labelImageCache;
@property (nonatomic) MHScaleBarLabel* prototypeLabel;
...
static const CGFloat MHBarHeight = 4;
static const CGFloat MHFeetPerMeter = 3.28084;
static const CGFloat MHScaleBarLabelWidthHint = 30.0;
static const CGFloat MHScaleBarMinimumBarWidth = 30.0; // Arbitrary

@interface MHScaleBarLabel : UILabel
@property (nonatomic) BOOL shouldShowDarkStyles;
@end

@implementation MHScaleBarLabel
...
- (void)commonInit {
    ...
    _containerView.layer.cornerRadius  = MHBarHeight / 2.0;
    _containerView.layer.masksToBounds = YES;

    [self addSubview:_containerView];
    
    _formatter = [[MHDistanceFormatter alloc] init];

    // Image labels are now images
    _labelImageCache              = [[NSMutableDictionary alloc] init];
    _prototypeLabel               = [[MHScaleBarLabel alloc] init];
    ...
    _lastLabelWidth = MHScaleBarLabelWidthHint;
...
- (CGFloat)actualWidth {
    ...
    if (width <= MHScaleBarMinimumBarWidth) {
        return 0.0;
    }
...
- (CGFloat)unitsPerPoint {
    return [self usesMetricSystem] ? self.metersPerPoint : self.metersPerPoint * MHFeetPerMeter;
}
...
- (MHRow)preferredRow {
    ...
    if (useMetric) {
        row = table = MHMetricTable;
        count = sizeof(MHMetricTable) / sizeof(MHMetricTable[0]);
    }
    else {
        row = table = MHImperialTable;
        count = sizeof(MHImperialTable) / sizeof(MHImperialTable[0]);
    }
...
- (void)updateVisibility {
    ...
    ? sizeof(MHMetricTable) / sizeof(MHMetricTable[0])
    : sizeof(MHImperialTable) / sizeof(MHImperialTable[0]);
    
    CLLocationDistance maximumDistance = [self maximumWidth] * [self unitsPerPoint];
    CLLocationDistance allowedDistance = metric
    ? MHMetricTable[count-1].distance
    : MHImperialTable[count-1].distance;
...
- (void)layoutSubviews {
    ...
    self.containerView.frame = CGRectMake(barOffset,
                                          intrinsicContentHeight - MHBarHeight,
                                          totalBarWidth,
                                          MHBarHeight);

    [self layoutBarsWithWidth:barWidth];
    
    CGFloat yPosition = round(0.5 * ( intrinsicContentHeight - MHBarHeight));
...
- (void)layoutBarsWithWidth:(CGFloat)barWidth {
    ...
    for (UIView *bar in self.bars) {
        ...
        bar.frame = CGRectMake(xPosition, 0, barWidth, MHBarHeight);
        i++;
    }
}
