...
#import "MHRenderingStats_Private.h"
#import "MHSettings_Private.h"
#import "MHActionJournalOptions_Private.h"
#import "MHMapProjection.h"
#import "MHPluginLayer.h"
#import "MHStyleLayerManager.h"
#include "MHPluginStyleLayer_Private.h"
...
@implementation MHMapView
{
    ...
    MHRenderingStats* _renderingStats;
}
...
- (instancetype)initWithFrame:(CGRect)frame styleJSON:(NSString *)styleJSON
{
    ...
    {
        MHLogInfo(@"Starting %@ initialization.", NSStringFromClass([self class]));
        MHLogDebug(@"Initializing frame: %@ styleJSON: %@", NSStringFromCGRect(frame), styleJSON);
        [self commonInitWithOptions:nil];
...
- (instancetype)initWithFrame:(CGRect)frame options:(MHMapOptions *)options
{
    ...
    {
        MHLogInfo(@"Starting %@ initialization.", NSStringFromClass([self class]));
        MHLogDebug(@"Initializing frame: %@ with options", NSStringFromCGRect(frame));
...
- (void)commonInitWithOptions:(MHMapOptions*)mlnMapoptions
{
    if (mlnMapoptions == nil)
    {
        mlnMapoptions = [[MHMapOptions alloc] init];
    }
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    if ([keyPath isEqualToString:@"coordinate"] && [object conformsToProtocol:@protocol(MHAnnotation)] && ![object isKindOfClass:[MHMultiPoint class]])
    {
        ...
        {
            const mbgl::Point<double> point = MHPointFromLocationCoordinate2D(annotation.coordinate);
...
{
    MHLogDebug(@"Setting quickZoomReversed: %@", MHStringFromBOOL(quickZoomReversed));
...
{
    MHLogDebug(@"Setting showsLogoView: %@", MHStringFromBOOL(showsLogoView));
...
{
    MHLogDebug(@"Setting showsCompassView: %@", MHStringFromBOOL(showsCompassView));
...
{
    MHLogDebug(@"Setting showsAttributionButton: %@", MHStringFromBOOL(showsAttributionButton));
...
{
    _mbglMap->setFrustumOffset(MHEdgeInsetsFromNSEdgeInsets(frustumOffset));
}

-(UIEdgeInsets)frustumOffset
{
    return NSEdgeInsetsFromMHEdgeInsets(_mbglMap->getFrustumOffset());
}
...
- (id)accessibilityElementForAnnotationWithTag:(MHAnnotationTag)annotationTag
{
    ...
    MHAnnotationContext &annotationContext = annotationContextIt->second;
...
- (void)setMaximumScreenBounds:(MHCoordinateBounds)maximumScreenBounds
{
...
- (MHCoordinateBounds)maximumScreenBounds
{
    return MHCoordinateBoundsFromLatLngBounds(*self.mbglMap.getBounds().bounds);;
}
...
- (void)_setDirection:(CLLocationDirection)direction center:(CLLocationCoordinate2D)center animated:(BOOL)animated
{
    ...
    else
    {
        ...
        {
            cameraOptions.center = MHLatLngFromLocationCoordinate2D(center);
                 ...
                 && ![annotation isKindOfClass:[MHPointCollection class]])
        {
            ...
            if (delegateImplementsViewForAnnotation)
            {
                ...
                if (annotationView)
                {
                    ...
                    annotationView.center = MHPointRounded([self convertCoordinate:annotation.coordinate toPointToView:self]);
            ...
            MHAnnotationTag annotationTag = self.mbglMap.addAnnotation(mbgl::SymbolAnnotation {
                MHPointFromLocationCoordinate2D(annotation.coordinate),
...
- (void)addOverlays:(NSArray<id <MHOverlay>> *)overlays
{
    ...
    {
        MHAssert([overlay conformsToProtocol:@protocol(MHOverlay)], @"overlay should conform to MHOverlay.h");
    }
#endif
    ...
    if (nearbyAnnotations.size())
    {
        ...
        auto end = std::remove_if(nearbyAnnotations.begin(), nearbyAnnotations.end(), [&](const MHAnnotationTag annotationTag) {
            ...
            if (annotationView)
            {
                ...
                CGPoint calloutAnchorPoint = MHPointRounded([self convertCoordinate:annotation.coordinate toPointToView:self]);
...
{
    CGPoint calloutAnchorPoint = MHPointRounded([self convertCoordinate:coordinate toPointToView:self]);
...
- (void)applyIconIdentifier:(NSString *)iconIdentifier toAnnotationsWithImageReuseIdentifier:(NSString *)reuseIdentifier
{
    for (auto &pair : _annotationContextsByAnnotationTag)
    {
        ...
        {
            const mbgl::Point<double> point = MHPointFromLocationCoordinate2D(pair.second.annotation.coordinate);
...
- (void)locationManager:(__unused id<MHLocationManager>)manager didUpdateLocations:(NSArray *)locations animated:(BOOL)animated completionHandler:(nullable void (^)(void))completion
{
    ...
    NSTimeInterval userLocationDuration = MHAnimationDuration;
    if (oldLocation && ! CGPointEqualToPoint(self.userLocationAnnotationView.center, CGPointZero))
    {
        userLocationDuration = MIN([newLocation.timestamp timeIntervalSinceDate:oldLocation.timestamp], MHUserLocationAnimationDuration);
    }
    [self updateUserLocationAnnotationViewAnimatedWithDuration:userLocationDuration];

    NSTimeInterval cameraDuration = MHUserLocationAnimationDuration;
    if (self.dynamicNavigationCameraAnimationDuration && oldLocation) {
        cameraDuration = MIN([newLocation.timestamp timeIntervalSinceDate:oldLocation.timestamp], MHUserLocationAnimationDuration);
    }
...
{
    [self didUpdateLocationWithUserTrackingDuration:animated ? MHUserLocationAnimationDuration : 0 completionHandler:completion];
}
...
- (void)didUpdateLocationSignificantlyAnimated:(BOOL)animated completionHandler:(nullable void (^)(void))completion
{
    ...
    [self _flyToCamera:camera
     ...
     completionHandler:^{
        ...
        // (The old second comparison against MHDistanceThresholdForCameraPause — a
        // CLLocationDistance of 100 — could never match a tracking state; newer
        // clang rejects the enum/double comparison outright.)
...
- (void)mapViewDidFinishRenderingFrameFullyRendered:(BOOL)fullyRendered
    ...
    {
        if (!_renderingStats) {
            _renderingStats = [[MHRenderingStats alloc] init];
        }
...
- (void)updateAnnotationViews
{
    ...
    for (id<MHAnnotation> annotation in visibleAnnotations)
    {
        ...
        if (annotationView)
        {
            annotationView.center = MHPointRounded([self convertCoordinate:annotationContext.annotation.coordinate toPointToView:self]);
        }
    }
...
- (void)updateUserLocationAnnotationViewAnimatedWithDuration:(NSTimeInterval)duration
{
    ...
    {
        userPoint = MHPointRounded([self convertCoordinate:self.userLocation.coordinate toPointToView:self]);
    }
...
-(void)addPluginLayerType:(Class)pluginLayerClass {
    ...
    MHPluginLayerCapabilities *capabilities = [pluginLayerClass layerCapabilities];
    ...
    __weak MHMapView *weakMapView = self;
    ...
    factory->setOnLayerCreatedEvent([layerClass, weakMapView, pluginLayerClass](mbgl::style::PluginLayer *pluginLayer) {
        ...
        MHPluginLayer *layer = [[layerClass alloc] init];
        ...
        MHPluginLayer *weakPlugInLayer = layer;
        ...
        for (MHPluginLayerProperty *property in capabilities.layerProperties) {
            mbgl::style::PluginLayerProperty *p = new mbgl::style::PluginLayerProperty();
            switch (property.propertyType) {
                case MHPluginLayerPropertyTypeSingleFloat:
                    ...
                    break;
                case MHPluginLayerPropertyTypeColor:
                {
        ...
        auto renderFunction = [weakPlugInLayer, weakMapView](mbgl::PaintParameters& paintParameters){
            ...
            MHMapView *strongMapView = weakMapView;

            const mbgl::TransformState& state = paintParameters.state;

            MHPluginLayerDrawingContext drawingContext;
            ...
            drawingContext.projectionMatrix = MHMatrix4Make(paintParameters.transformParams.projMatrix);
            drawingContext.nearClippedProjMatrix = MHMatrix4Make(paintParameters.transformParams.nearClippedProjMatrix);
