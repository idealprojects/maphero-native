...
@interface MHCompassButton ()

@property (nonatomic, weak) MHMapView *mapView;
@property (nonatomic) MHCompassDirectionFormatter *accessibilityCompassFormatter;

@end

@implementation MHCompassButton

+ (instancetype)compassButtonWithMapView:(MHMapView *)mapView {
    return [[MHCompassButton alloc] initWithMapView:mapView];
}
...
- (void)commonInit {
    ...
    self.compassVisibility = MHOrnamentVisibilityAdaptive;
    ...
    self.accessibilityCompassFormatter = [[MHCompassDirectionFormatter alloc] init];
    self.accessibilityCompassFormatter.unitStyle = NSFormattingUnitStyleLong;

    [self sizeToFit];
}

- (void)setCompassVisibility:(MHOrnamentVisibility)compassVisibility {
...
- (void)updateCompassAnimated:(BOOL)animated {
    ...
    self.transform = CGAffineTransformMakeRotation(MHRadiansFromDegrees(plateDirection));
    ...
    switch (self.compassVisibility) {
        case MHOrnamentVisibilityAdaptive:
            ...
            break;
        case MHOrnamentVisibilityVisible:
            [self showCompass:animated];
            break;
        case MHOrnamentVisibilityHidden:
            [self hideCompass:animated];
            break;
    }
}
...
- (void)animateToAlpha:(CGFloat)alpha {
    [UIView animateWithDuration:MHAnimationDuration delay:0 options:UIViewAnimationOptionBeginFromCurrentState animations:^{
         self.alpha = alpha;
     } completion:nil];
}
