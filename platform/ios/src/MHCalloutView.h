...
@protocol MHCalloutViewDelegate;
...
/**
 ...
 To receive updates from an object that conforms to the ``MHCalloutView`` protocol,
 use the optional methods available in the ``MHCalloutViewDelegate`` protocol.

 #### Related examples
 TODO: Display custom views as callouts by customizing an
 ``MHCalloutView``.
 */
@protocol MHCalloutView <NSObject>

/**
 An object conforming to the ``MHAnnotation`` protocol whose details this callout
 view displays.
 */
...
/**
 An object conforming to the ``MHCalloutViewDelegate`` method that receives
...
/**
 ...
 ``MHCalloutView/presentCalloutFromRect:inView:constrainedToRect:animated:``
...
/**
 The ``MHCalloutViewDelegate`` protocol defines a set of optional methods that
 you can use to receive messages from an object that conforms to the
 ``MHCalloutView`` protocol. The callout view uses these methods to inform the
 delegate that the user has interacted with the the callout view.
 */
@protocol MHCalloutViewDelegate <NSObject>
 ...
 */
- (BOOL)calloutViewShouldHighlight:(UIView<MHCalloutView> *)calloutView;

/**
 Tells the delegate that the callout view has been tapped.
 */
- (void)calloutViewTapped:(UIView<MHCalloutView> *)calloutView;
