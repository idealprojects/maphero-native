...
#import "MHUserLocationHeadingBeamLayer.h"
#import "MHLocationManager_Private.h"
#import "MHUserLocationAnnotationViewStyle.h"

const CGFloat MHUserLocationAnnotationDotSize = 22.0;
const CGFloat MHUserLocationAnnotationHaloSize = 115.0;

const CGFloat MHUserLocationAnnotationPuckSize = 45.0;
const CGFloat MHUserLocationAnnotationArrowSize = MHUserLocationAnnotationPuckSize * 0.5;

const CGFloat MHUserLocationHeadingUpdateThreshold = 0.01;

const CGFloat MHUserLocationApproximateZoomThreshold = 7.0;

@implementation MHFaux3DUserLocationAnnotationView
{
    ...
    CALayer<MHUserLocationHeadingIndicator> *_headingIndicatorLayer;
...
- (void)update
{
    ...
    {
        CGFloat frameSize = (self.mapView.userTrackingMode == MHUserTrackingModeFollowWithCourse) ? MHUserLocationAnnotationPuckSize : MHUserLocationAnnotationDotSize;
...
- (void)drawPreciseLocationPuck {
    ...
    (self.mapView.userTrackingMode == MHUserTrackingModeFollowWithCourse) ? [self drawPuck] : [self drawDot];
    [self updatePitch];
...
- (void)setTintColor:(UIColor *)tintColor
{
    ...
    if ([self.mapView.delegate respondsToSelector:@selector(mapViewStyleForDefaultUserLocationAnnotationView:)]) {
        MHUserLocationAnnotationViewStyle *style = [self.mapView.delegate mapViewStyleForDefaultUserLocationAnnotationView:self.mapView];
...
- (void)updatePitch
{
    if (self.mapView.camera.pitch != _oldPitch)
    {
        ...
        CATransform3D t = CATransform3DRotate(CATransform3DIdentity, MHRadiansFromDegrees(self.mapView.camera.pitch), 1.0, 0, 0);
...
{
    CGFloat pitch = MHRadiansFromDegrees(self.mapView.camera.pitch);
...
- (void)drawPuck
{
    if ( ! _puckModeActivated)
    {
        ...
        [self updateFrameWithSize:MHUserLocationAnnotationPuckSize];
    }
    ...
    if ( ! _puckDot)
    {
        _puckDot = [self circleLayerWithSize:MHUserLocationAnnotationPuckSize];
    ...
    if ( ! _puckArrow)
    {
        ...
        _puckArrow.bounds = CGRectMake(0, 0, round(MHUserLocationAnnotationArrowSize), round(MHUserLocationAnnotationArrowSize));
    ...
    {
        _puckArrow.affineTransform = CGAffineTransformRotate(CGAffineTransformIdentity, -MHRadiansFromDegrees(self.mapView.direction - self.userLocation.location.course));
    }
...
{
    CGFloat max = MHUserLocationAnnotationArrowSize;
...
- (void)drawDot
{
    if (_puckModeActivated)
    {
        ...
        [self updateFrameWithSize:MHUserLocationAnnotationDotSize];
    }
    ...
    //
    BOOL headingTrackingModeEnabled = self.mapView.userTrackingMode == MHUserTrackingModeFollowWithHeading;
    ...
    if (showHeadingIndicator)
    {
        ...
        if (([_headingIndicatorLayer isMemberOfClass:[MHUserLocationHeadingBeamLayer class]] && ! headingTrackingModeEnabled) ||
            ([_headingIndicatorLayer isMemberOfClass:[MHUserLocationHeadingArrowLayer class]] && headingTrackingModeEnabled))
        {
        ...
        if ( ! _headingIndicatorLayer && headingAccuracy)
        {
            ...
            {
                _headingIndicatorLayer = [[MHUserLocationHeadingBeamLayer alloc] initWithUserLocationAnnotationView:self];
            ...
            {
                _headingIndicatorLayer = [[MHUserLocationHeadingArrowLayer alloc] initWithUserLocationAnnotationView:self];
        ...
        {
            CGFloat rotation = -MHRadiansFromDegrees(self.mapView.direction - headingDirection);

            // Don't rotate if the change is imperceptible.
            if (fabs(rotation) > MHUserLocationHeadingUpdateThreshold)
            {
                [CATransaction begin];
                [CATransaction setDisableActions:YES];
    ...
    if (_accuracyRingLayer && (_oldZoom != self.mapView.zoomLevel || _oldHorizontalAccuracy != self.userLocation.location.horizontalAccuracy))
    {
        ...
        if (accuracyRingSize > MHUserLocationAnnotationDotSize + 15)
        {
        ...
        else
        {
            ...
            _haloLayer.bounds = CGRectMake(0, 0, MHUserLocationAnnotationHaloSize, MHUserLocationAnnotationHaloSize);
            _haloLayer.cornerRadius = MHUserLocationAnnotationHaloSize / 2.0;
    ...
    if ( ! _haloLayer)
    {
        _haloLayer = [self circleLayerWithSize:MHUserLocationAnnotationHaloSize];
    ...
    if ( ! _dotBorderLayer)
    {
        _dotBorderLayer = [self circleLayerWithSize:MHUserLocationAnnotationDotSize];
    ...
    if ( ! _dotLayer)
    {
        _dotLayer = [self circleLayerWithSize:MHUserLocationAnnotationDotSize * 0.75];
...
- (void)drawApproximate
{
    ...
    {
        CGFloat accuracyRingSize = [self calculateAccuracyRingSize: MAX(self.mapView.zoomLevel, MHUserLocationApproximateZoomThreshold)];
    ...
    {
        if (self.mapView.zoomLevel < MHUserLocationApproximateZoomThreshold) {
            borderSize = 1.0;
        }
        _approximateLayer.borderWidth = borderSize;
