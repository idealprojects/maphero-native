...
@protocol MHOverlay;
@protocol MHCalloutView;
@protocol MHFeature;
@protocol MHLocationManager;

/** Options for ``MHMapView/decelerationRate``. */
typedef CGFloat MHMapViewDecelerationRate NS_TYPED_EXTENSIBLE_ENUM;

/** The default deceleration rate for a map view. */
FOUNDATION_EXTERN MH_EXPORT const MHMapViewDecelerationRate MHMapViewDecelerationRateNormal;

/** A fast deceleration rate for a map view. */
FOUNDATION_EXTERN MH_EXPORT const MHMapViewDecelerationRate MHMapViewDecelerationRateFast;

/** Disables deceleration in a map view. */
FOUNDATION_EXTERN MH_EXPORT const MHMapViewDecelerationRate MHMapViewDecelerationRateImmediate;

/**
 The vertical alignment of an annotation within a map view. Used with
 ``MHMapView/userLocationVerticalAlignment``.
 */
typedef NS_ENUM(NSUInteger, MHAnnotationVerticalAlignment) {
  /** Aligns the annotation vertically in the center of the map view. */
  MHAnnotationVerticalAlignmentCenter = 0,
  /** Aligns the annotation vertically at the top of the map view. */
  MHAnnotationVerticalAlignmentTop,
  /** Aligns the annotation vertically at the bottom of the map view. */
  MHAnnotationVerticalAlignmentBottom,
};

/**
 The position of scale bar, compass, logo and attribution in a map view. Used with
 ``MHMapView/scaleBarPosition``,
 ``MHMapView/compassViewPosition``,
 ``MHMapView/logoViewPosition``,
 ``MHMapView/attributionButtonPosition``.
 */
typedef NS_ENUM(NSUInteger, MHOrnamentPosition) {
  /** Place the ornament in the top left of the map view. */
  MHOrnamentPositionTopLeft = 0,
...
/**
 ...
 ``MHMapView/userTrackingMode``.

 #### Related examples
 - TODO: Switch between user tracking modes</a> example to learn how to toggle modes and
 how each mode behaves.
 */
typedef NS_ENUM(NSUInteger, MHUserTrackingMode) {
  /** The map does not follow the user location. */
  MHUserTrackingModeNone = 0,
  /** The map follows the user location. This tracking mode falls back
      to ``MHUserTrackingMode/MHUserTrackingModeNone`` if the user pans the map view. */
  MHUserTrackingModeFollow,
  /**
      ...
      view, this tracking mode will fall back to ``MHUserTrackingModeFollow``.
   */
  MHUserTrackingModeFollowWithHeading,
  /**
      ...
      this tracking mode will fall back to ``MHUserTrackingModeFollow``.
   */
  MHUserTrackingModeFollowWithCourse,
};

typedef NS_ENUM(NSUInteger, MHPanScrollingMode) {
  /** The map allows the user to only scroll horizontally. */
  MHPanScrollingModeHorizontal = 0,
  /** The map allows the user to only scroll vertically. */
  MHPanScrollingModeVertical,
  /** The map allows the user to scroll both horizontally and vertically. */
  MHPanScrollingModeDefault
};

/** Options for ``MHMapView/preferredFramesPerSecond``. */
typedef NSInteger MHMapViewPreferredFramesPerSecond NS_TYPED_EXTENSIBLE_ENUM;

/**
 The default frame rate. This can be either 30 FPS or 60 FPS, depending on
 device capabilities.
 */
FOUNDATION_EXTERN MH_EXPORT const MHMapViewPreferredFramesPerSecond
    MHMapViewPreferredFramesPerSecondDefault;

/** A conservative frame rate; typically 30 FPS. */
FOUNDATION_EXTERN MH_EXPORT const MHMapViewPreferredFramesPerSecond
    MHMapViewPreferredFramesPerSecondLowPower;

/** The maximum supported frame rate; typically 60 FPS. */
FOUNDATION_EXTERN MH_EXPORT const MHMapViewPreferredFramesPerSecond
    MHMapViewPreferredFramesPerSecondMaximum;

FOUNDATION_EXTERN MH_EXPORT
    MHExceptionName const MHMissingLocationServicesUsageDescriptionException;
FOUNDATION_EXTERN MH_EXPORT MHExceptionName const MHUserLocationAnnotationTypeException;

/**
 An interactive, customizable map view with an interface similar to the one
 provided by Apple’s MapKit.

 Using ``MHMapView``, you can embed the map inside a view, allow users to
 ...
 Because ``MHMapView`` loads asynchronously, several delegate methods are available
 ...
 in the ``MHMapViewDelegate`` protocol documentation.

 Adding your own gesture recognizer to ``MHMapView`` will block the corresponding
 gesture recognizer built into ``MHMapView``. To avoid conflicts, define which
 gesture takes precedence. For example, you can create your own
 `UITapGestureRecognizer` that will be invoked only if the default ``MHMapView``
 tap gesture fails:
 ...
 */
MH_EXPORT
@interface MHMapView : UIView <MHStylable>
...
/**
 ...
 - TODO: initialize an ``MHMapView`` with a custom style
 - TODO: how to initialize an ``MHMapView`` with a third-party tile source
 */
...
/**
 ...
 annotations.
 */
@property (nonatomic, weak, nullable) IBOutlet id<MHMapViewDelegate> delegate;
...
/**
 ...
 ``MHMapViewDelegate/mapViewDidFinishLoadingMap:`` method. It is not possible
 ...
 */
@property (nonatomic, readonly) MHScaleBar *scaleBar;
...
/**
 ...
 ``MHOrnamentPosition/MHOrnamentPositionTopLeft``.
 */
@property (nonatomic, assign) MHOrnamentPosition scaleBarPosition;
 ...
 */
@property (nonatomic, readonly) MHCompassButton *compassView;

/**
 The position of the compass view. The default value is
 ``MHOrnamentPosition/MHOrnamentPositionTopRight``.
 */
@property (nonatomic, assign) MHOrnamentPosition compassViewPosition;
...
/**
 ...
 ``MHOrnamentPosition/MHOrnamentPositionBottomLeft``.
 */
@property (nonatomic, assign) MHOrnamentPosition logoViewPosition;
...
/**
 ...
 ``MHOrnamentPosition/MHOrnamentPositionBottomRight``.
 */
@property (nonatomic, assign) MHOrnamentPosition attributionButtonPosition;
...
/**
 ...
 ``MHMapViewPreferredFramesPerSecondDefault``, which will adaptively set the
 preferred frame rate based on the capability of the user’s device to maintain
 a smooth experience.

 In addition to the provided ``MHMapViewPreferredFramesPerSecond`` options, this
 property can be set to arbitrary integer values.

 @see `CADisplayLink.preferredFramesPerSecond`
 */
@property (nonatomic, assign) MHMapViewPreferredFramesPerSecond preferredFramesPerSecond;
...
/**
 ...
 ``MHMapViewDelegate/mapView:didUpdateUserLocation:`` and
 ``MHMapViewDelegate/mapView:didFailToLocateUserWithError:`` methods.

 If setting this property to `nil` or if no custom manager is provided this
 property is set to the default location manager.

 ``MHMapView`` uses a default location manager. If you want to substitute your
 ...
 */
@property (nonatomic, null_resettable) id<MHLocationManager> locationManager;
 ...
 */
@property (nonatomic, readonly, nullable) MHUserLocation *userLocation;

/**
 The mode used to track the user location. The default value is
 ``MHUserTrackingMode/MHUserTrackingModeNone``.
 ...
 default user location annotation shown by ``MHUserTrackingMode``.
 */
@property (nonatomic, assign) MHUserTrackingMode userTrackingMode;
...
/**
 ...
 default value is ``MHAnnotationVerticalAlignment/MHAnnotationVerticalAlignmentCenter``.
...
/**
 ...
 ``MHUserTrackingMode/MHUserTrackingModeFollowWithHeading``.

 This property has no effect when ``userTrackingMode`` is
 ``MHUserTrackingMode/MHUserTrackingModeFollowWithHeading`` or
 ``MHUserTrackingMode/MHUserTrackingModeFollowWithCourse``.
...
/**
 ...
 value other than ``MHUserTrackingMode/MHUserTrackingModeFollowWithCourse``.
...
/**
 ...
 other than ``MHUserTrackingMode/MHUserTrackingModeFollowWithCourse``.
...
`MHPanScrollingModeHorizontal` only allows the user to scroll horizontally on the map,
 restricting a user's ability to scroll vertically.
`MHPanScrollingModeVertical` only allows the user to scroll vertically on the map,
 restricting a user's ability to scroll horizontally.
 ``MHPanScrollingMode/MHPanScrollingModeDefault`` allows the user to scroll both horizontally and
vertically on the map.

 By default, this property is set to ``MHPanScrollingMode/MHPanScrollingModeDefault``.
 */
@property (nonatomic, assign) MHPanScrollingMode panScrollingMode;
...
/**
 ...
 Your application can use the ``MHMapViewDecelerationRateNormal`` and
 ``MHMapViewDecelerationRateFast`` constants as reference points for reasonable
 deceleration rates. ``MHMapViewDecelerationRateImmediate`` can be used to
 ...
 */
@property (nonatomic) MHCoordinateBounds visibleCoordinateBounds;
...
/**
 ...
 @param animated `YES` if you want the map region change to be animated, or `NO`
    if you want the map to display the new region immediately without animations.
 */
- (void)showAnnotations:(NSArray<id<MHAnnotation>> *)annotations
            edgePadding:(UIEdgeInsets)insets
               animated:(BOOL)animated
    __attribute__((
 ...
 */
@property (nonatomic, copy) MHMapCamera *camera;
...
- (MHMapCamera *)camera:(MHMapCamera *)camera
    fittingCoordinateBounds:(MHCoordinateBounds)bounds
                edgePadding:(UIEdgeInsets)insets;
...
/**
 ...
 depending on the order of notification delivery.
 */
- (MHMapCamera *)cameraThatFitsShape:(MHShape *)shape
                            direction:(CLLocationDirection)direction
                          edgePadding:(UIEdgeInsets)insets;
...
/**
 ...
 If the ``userTrackingMode`` property is not ``MHUserTrackingMode/MHUserTrackingModeNone``, the
...
/**
 ...
 use the map view’s property ``MHMapView/automaticallyAdjustsContentInset``instead.
...
/**
 ...
 The objects in this array must adopt the ``MHAnnotation`` protocol. If no
 annotations are associated with the map view, the value of this property is
 `nil`.
 */
@property (nonatomic, readonly, nullable) NSArray<id<MHAnnotation>> *annotations;

/**
 Adds an annotation to the map view.

 > Note: ``MHMultiPolyline``, ``MHMultiPolyline``, ``MHMultiPolyline``, and
    ``MHPointCollection`` objects cannot be added to the map view at this time.
 ...
 @param annotation The annotation object to add to the receiver. This object
    must conform to the ``MHAnnotation`` protocol. The map view retains the
    annotation object.

 #### Related examples
 - TODO: add a line annotation from GeoJSON.
 - TODO: add an annotation to an ``MHMapView`` object.
 */
...
/**
 ...
 > Note: ``MHMultiPolyline``, ``MHMultiPolyline``, and ``MHMultiPolyline`` objects
    cannot be added to the map view at this time. Nor can ``MHMultiPoint``
 ...
 @param annotations An array of annotation objects. Each object in the array
    must conform to the ``MHAnnotation`` protocol. The map view retains each
...
/**
 ...
 @param annotations The array of annotation objects to remove. Objects in the
    array must conform to the ``MHAnnotation`` protocol.
 */
- (void)removeAnnotations:(NSArray<id<MHAnnotation>> *)annotations;

/**
 Returns an ``MHAnnotationView`` if the given annotation is currently associated
 with a view, otherwise nil.

 @param annotation The annotation associated with the view.
    Annotation must conform to the ``MHAnnotation`` protocol.
 */
...
/**
 ...
 For performance reasons, you should generally reuse ``MHAnnotationImage``
 ...
 reuse an ``MHAnnotationImage``.
 */
- (nullable __kindof MHAnnotationImage *)dequeueReusableAnnotationImageWithIdentifier:
    (NSString *)identifier;

/**
 Returns a reusable annotation view object associated with its identifier.

 For performance reasons, you should generally reuse ``MHAnnotationView``
 objects for identical-looking annotations in your map views. Dequeueing
 saves time and memory during performance-critical operations such as
 scrolling.
 ...
 @return An annotation view object with the given identifier, or `nil` if no
    such object exists in the reuse queue.
 */
- (nullable __kindof MHAnnotationView *)dequeueReusableAnnotationViewWithIdentifier:
    (NSString *)identifier;
 ...
 */
@property (nonatomic, readonly, nullable) NSArray<id<MHAnnotation>> *visibleAnnotations;
...
/**
 ...
 @return An array of objects that adopt the ``MHAnnotation`` protocol or `nil` if
...
/**
 ...
 If the annotation is of type ``MHPointAnnotation`` and is offscreen, the camera
 ...
 selected.
 */
@property (nonatomic, copy) NSArray<id<MHAnnotation>> *selectedAnnotations;
...
/**
 ...
 annotation is not visible (or is partially visible) *and* is of type ``MHPointAnnotation``, the
...
/**
 ...
 ``MHPointAnnotation``, the map is panned so that the annotation and its callout are brought into
 ...
 */
- (void)deselectAnnotation:(nullable id<MHAnnotation>)annotation animated:(BOOL)animated;
...
/**
 ...
 empty array.
 */
@property (nonatomic, readonly, nonnull) NSArray<id<MHOverlay>> *overlays;
...
/**
 ...
 ``MHMapView`` object for visible ``MHMapView`` objects.
 */
...
/**
 ...
 ``MHShapeSource`` and ``MHShapeSource`` objects but does not include
 anything from ``MHRasterTileSource`` objects, or from video or canvas sources,
 ...
 ``MHFeature`` object representing that bus stop is returned and its
 ...
 visibility, use the
 ``MHVectorTileSource/featuresInSourceLayersWithIdentifiers:predicate:`` and
 ``MHShapeSource/featuresMatchingPredicate:`` methods on the relevant sources.

 The returned features may also include features corresponding to annotations.
 These features are not object-equal to the ``MHAnnotation`` objects that were
 originally added to the map. To query the map for annotations, use
 `visibleAnnotations` or ``MHMapView/visibleAnnotationsInRect:``.
...
/**
 ...
 @return An array of objects conforming to the ``MHFeature`` protocol that
    represent features in the sources used by the current style.
 */
- (NSArray<id<MHFeature>> *)visibleFeaturesInRect:(CGRect)rect NS_SWIFT_NAME(visibleFeatures(in:));
...
/**
 ...
 array. On the other hand, if the style does include bus stops, an ``MHFeature``
