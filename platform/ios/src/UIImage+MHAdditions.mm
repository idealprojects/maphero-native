...
const MHExceptionName MHResourceNotFoundException = @"MHResourceNotFoundException";

BOOL MHEdgeInsetsIsZero(UIEdgeInsets edgeInsets) {
    return edgeInsets.left == 0 && edgeInsets.top == 0 && edgeInsets.right == 0 && edgeInsets.bottom == 0;
}

@implementation UIImage (MHAdditions)

- (nullable instancetype)initWithMHStyleImage:(const mbgl::style::Image &)styleImage
{
    CGImageRef image = CGImageCreateWithMHPremultipliedImage(styleImage.getImage().clone());
    if (!image) {
        return nil;
    }
...
- (nullable instancetype)initWithMHPremultipliedImage:(const mbgl::PremultipliedImage&&)mbglImage scale:(CGFloat)scale
{
    CGImageRef image = CGImageCreateWithMHPremultipliedImage(mbglImage.clone());
    if (!image) {
        return nil;
    }
...
- (std::unique_ptr<mbgl::style::Image>)mgl_styleImageWithIdentifier:(NSString *)identifier {
    ...
    if (!MHEdgeInsetsIsZero(self.capInsets))
    {
...
- (mbgl::PremultipliedImage)mgl_premultipliedImage {
    return MHPremultipliedImageFromCGImage(self.CGImage);
}
...
+ (UIImage *)mgl_resourceImageNamed:(NSString *)imageName {
    ...
    if (!image) {
        [NSException raise:MHResourceNotFoundException format:@"The resource named “%@” could not be found in the Mapbox framework bundle.", imageName];
    }
