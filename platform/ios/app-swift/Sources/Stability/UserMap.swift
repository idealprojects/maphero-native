...
class UserMap: MHMapView, MHMapViewDelegate {
    ...
    func randomAltitude() -> Double {
        MHAltitudeForZoomLevel(randomZoom(), camera.pitch, camera.centerCoordinate.latitude, frame.size)
    }
    ...
    func mapView(_ mapView: MHMapView, viewFor annotation: MHAnnotation) -> MHAnnotationView? {
        ...
        class AnnotationView: MHAnnotationView {
            var map: UserMap
    ...
    func mapView(_: MHMapView, strokeColorForShapeAnnotation _: MHShape) -> UIColor {
        randomColor()
    }

    func mapView(_: MHMapView, lineWidthForPolylineAnnotation _: MHPolyline) -> CGFloat {
        randomLineWidth()
    }

    func mapView(_: MHMapView, fillColorForPolygonAnnotation _: MHPolygon) -> UIColor {
        randomColor()
    }
    ...
    func runAnnotationActions() {
        ...
        randomAnnotationAdd().repeat {
            ...
            let annotation = MHPolyline(coordinates: coords, count: UInt(coords.count))
        ...
        randomAnnotationAdd().repeat {
            let coords = randomPolyPoints()
            let annotation = MHPolygon(coordinates: coords, count: UInt(coords.count))
