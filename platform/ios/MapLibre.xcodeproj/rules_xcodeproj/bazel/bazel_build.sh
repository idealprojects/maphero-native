...
bazel_cmd=(
  ...
  "/Users/khalidtaha/Library/Caches/bazelisk/downloads/sha256/ce73346274c379f77880db8bd8b9c8569885fe56f19386173760949da9078df0/bin/bazel"
...
"$BAZEL_INTEGRATION_DIR/process_bazel_build_log.py" \
  ...
  "$output_groups_flag" \
  "@@rules_xcodeproj++internal+rules_xcodeproj_generated//generator/platform/ios/xcodeproj:xcodeproj" \
...
if [[ -n "${target_ids:-}" ]]; then
  if [[ ! -s "$PROJECT_DIR/bazel-out/darwin_x86_64-dbg/bin/external/rules_xcodeproj++internal+rules_xcodeproj_generated/generator/platform/ios/xcodeproj/xcodeproj_target_ids" ]]; then
      echo "error: \"$PROJECT_DIR/bazel-out/darwin_x86_64-dbg/bin/external/rules_xcodeproj++internal+rules_xcodeproj_generated/generator/platform/ios/xcodeproj/xcodeproj_target_ids\" was not created. This can happen if" \
...
"https://github.com/MobileNativeFoundation/rules_xcodeproj/issues/new?template=bug.md" \
  ...
  diff_output=$(
    comm -23 \
      <(printf '%s\n' "${target_ids[@]}" | sort) \
      <(sort "$PROJECT_DIR/bazel-out/darwin_x86_64-dbg/bin/external/rules_xcodeproj++internal+rules_xcodeproj_generated/generator/platform/ios/xcodeproj/xcodeproj_target_ids")
  )
