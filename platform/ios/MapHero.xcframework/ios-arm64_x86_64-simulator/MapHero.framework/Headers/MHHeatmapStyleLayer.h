...
/**
 An ``MHHeatmapStyleLayer`` is a style layer that renders a <a
 ...
 features can come from vector tiles loaded by an ``MHHeatmapStyleLayer``
 object, or they can be ``MHHeatmapStyleLayer``, ``MHHeatmapStyleLayer``,
 ``MHHeatmapStyleLayer``, or ``MHHeatmapStyleLayer`` instances in an
 ``MHHeatmapStyleLayer`` or ``MHHeatmapStyleLayer`` object.
 
 Consider accompanying a heatmap style layer with an ``MHHeatmapStyleLayer`` or
 ``MHHeatmapStyleLayer`` at high zoom levels. If you are unsure whether the
 point data in an ``MHHeatmapStyleLayer`` is dense enough to warrant a heatmap,
 you can alternatively cluster the source using the ``MHHeatmapStyleLayer``
 option and render the data using an ``MHHeatmapStyleLayer`` or
 ``MHHeatmapStyleLayer``.
 ...
 */
MH_EXPORT
@interface MHHeatmapStyleLayer : MHVectorStyleLayer
...
*/
@property (nonatomic) MHTransition heatmapIntensityTransition;
...
*/
@property (nonatomic) MHTransition heatmapOpacityTransition;
...
*/
@property (nonatomic) MHTransition heatmapRadiusTransition;
