...
/**
 The ``MHMultiPoint`` class is an abstract superclass used to define shapes
 composed of multiple vertices.

 Create instances of ``MHPolyline`` or ``MHPolyline`` in order to use
 properties of ``MHMultiPoint``. Do not create instances of ``MHMultiPoint``
 ...
 Do not confuse ``MHMultiPoint`` with ``MHMultiPoint``, which represents a
 collection of related but disconnected points.
 */
MH_EXPORT
@interface MHMultiPoint : MHShape
...
/**
 ...
 immediately. If the shape is part of an ``MHShapeSource`` object, you must
 explicitly set the ``MHShapeSource/shape`` property in order for any style
