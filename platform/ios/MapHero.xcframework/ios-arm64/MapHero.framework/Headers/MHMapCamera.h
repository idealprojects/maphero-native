...
/**
 An ``MHMapCamera`` object represents a viewpoint from which the user observes
 some point on an ``MHMapView``.

 #### Related examples
 - <doc:BlockingGesturesExample>: learn how to use the
 ``MHMapViewDelegate/mapView:shouldChangeFromCamera:toCamera:`` method of ``MHMapViewDelegate`` to
 restrict panning.
 - *TODO:* Camera animation, learn how to create a camera that rotates
 around a central point.
 */
MH_EXPORT
@interface MHMapCamera : NSObject <NSSecureCoding, NSCopying>
