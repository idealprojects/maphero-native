 ...
 */
typedef NS_ENUM(NSUInteger, MHLightAnchor) {
    /**
 ...
 */
typedef struct __attribute__((objc_boxable)) MHSphericalPosition {
    /** Distance from the center of the base of an object to its light. */
    CGFloat radial;
    /** Position of the light relative to 0° (0° when ``MHLight/anchor`` is set to viewport corresponds
     to the top of the viewport, or 0° when ``MHLight/anchor`` is set to map corresponds to due north,
...
/**
 Creates a new ``MHSphericalPosition`` from the given radial, azimuthal, polar.

 @param radial The radial coordinate.
 @param azimuthal The azimuthal angle.
 @param polar The polar angle.

 @return Returns a ``MHSphericalPosition`` struct containing the position attributes.
 */
NS_INLINE MHSphericalPosition MHSphericalPositionMake(CGFloat radial, CLLocationDirection azimuthal, CLLocationDirection polar) {
...
/**
  An ``MHLight`` object represents the light source for extruded geometries in
 ``MHLight``.
 ...
 * Step functions applied to the `$zoomLevel` variable
 ...
 light property in the MapHero Style Spec.
 */
@property (nonatomic) NSExpression *anchor;

/**
 Position of the ``MHLight`` source relative to lit (extruded) geometries, in a
 ``MHLight`` struct [radial coordinate, azimuthal angle, polar angle] where
 ...
 ``MHLight/anchor`` is set to ``MHLight`` corresponds to the top of the
 viewport, or 0° when ``MHLight/anchor`` is set to ``MHLight`` corresponds to
 ...
 ``MHSphericalPosition`` struct set to 1.15 radial, 210 azimuthal and 30 polar.
 
 You can set this property to an expression containing any of the following:
 
 * Constant ``MHSphericalPosition`` values
 * Predefined functions, including mathematical and string operators
 * Conditional expressions
 * Variable assignments and references to assigned variables
 * Interpolation and step functions applied to the `$zoomLevel` variable
 ...
 value of an ``MHLight`` object for 3D geometries.
 */
...
*/
@property (nonatomic) MHTransition positionTransition;
...
*/
@property (nonatomic) MHTransition colorTransition;
...
*/
@property (nonatomic) MHTransition intensityTransition;
