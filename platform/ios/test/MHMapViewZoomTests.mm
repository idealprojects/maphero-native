...
@interface MHMapView (MHMapViewZoomTests)
...
@interface MHMapViewZoomTests : XCTestCase
@property (nonatomic) MHMapView *mapView;
@end

@implementation MHMapViewZoomTests
...
- (void)testPinchGesture {
    ...
    for (NSNumber *zoom in @[@1, @5, @10, @15, @22]) {
        gesture.state = UIGestureRecognizerStateChanged;
        gesture.scale = MHScaleFromZoomLevel([zoom doubleValue]);
        [self.mapView handlePinchGesture:gesture];
...
- (void)testPinchGestureOffCenter {
    ...
    gesture.state = UIGestureRecognizerStateChanged;
    gesture.scale = MHScaleFromZoomLevel(3);
    [self.mapView handlePinchGesture:gesture];
...
- (void)testPinchAndZoom {
    ...
    [[NSUserDefaults standardUserDefaults] setObject:@3 forKey:@"MHRotationThresholdWhileZooming"];
    ...
    rotate.rotation = MHRadiansFromDegrees(1);
    ...
    rotate.rotation = MHRadiansFromDegrees(2);
    [self.mapView handleRotateGesture:rotate];
    ...
    for (NSNumber *degrees in @[@-90, @-10, @10, @10, @30, @90, @180, @240, @460, @500, @590, @800]) {
        rotate.state = UIGestureRecognizerStateChanged;
        rotate.rotation = MHRadiansFromDegrees([degrees doubleValue]);
        [self.mapView handleRotateGesture:rotate];

        CGFloat wrappedRotation = mbgl::util::wrap(-MHDegreesFromRadians(rotate.rotation), 0., 360.);
...
NS_INLINE CGFloat MHScaleFromZoomLevel(double zoom) {
    return pow(2, zoom);
}

__unused NS_INLINE double MHZoomLevelFromScale(CGFloat scale) {
    return log2(scale);
}
