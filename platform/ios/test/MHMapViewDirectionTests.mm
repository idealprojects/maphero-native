...
@interface MHMapView (MHMapViewDirectionTests)
- (void)handleRotateGesture:(UIRotationGestureRecognizer *)rotate;
- (void)resetNorthAnimated:(BOOL)animated;
@end


@interface MHMapViewDirectionTests : XCTestCase
@property (nonatomic) MHMapView *mapView;
@end

@implementation MHMapViewDirectionTests
...
- (void)setUp {
    ...
    self.mapView = [[MHMapView alloc] initWithFrame:CGRectMake(0, 0, 64, 64) styleURL:styleURL];
}
...
- (void)testRotateEnabled {
    ...
    gesture.rotation = MHRadiansFromDegrees(30);
    CGFloat wrappedRotation = mbgl::util::wrap(-MHDegreesFromRadians(gesture.rotation), 0., 360.);
...
- (void)testRotationGesture {
    ...
    for (NSNumber *degrees in @[@-999, @-360, @-240, @-180, @-90, @-45, @0, @45, @90, @180, @240, @359, @999]) {
        gesture.state = UIGestureRecognizerStateChanged;
        gesture.rotation = MHRadiansFromDegrees([degrees doubleValue]);
        [self.mapView handleRotateGesture:gesture];
        CGFloat wrappedRotation = mbgl::util::wrap(-MHDegreesFromRadians(gesture.rotation), 0., 360.);
...
- (void)testResetPosition {
    ...
    MHMapCamera *defaultCamera = [MHMapCamera cameraLookingAtCenterCoordinate:CLLocationCoordinate2DMake(0, 0) altitude:self.mapView.camera.altitude pitch:0 heading:0];
