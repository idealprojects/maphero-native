...
- (void)setTiltGestureYTranslationForPitchDegrees:(CGFloat)degrees {
    // The tilt gesture takes the number of screen points the fingers have moved and then divides them by a "slowdown" factor, which happens to be set to 2.0 in -[MHMapView handleTwoFingerDragGesture:].
...
- (void)setUp {
    ...
    self.mapView = [[MHMapView alloc] initWithFrame:CGRectMake(0, 0, 400, 400) styleURL:styleURL];
}
