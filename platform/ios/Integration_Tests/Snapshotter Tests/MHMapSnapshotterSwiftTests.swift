...
class MHMapSnapshotterSwiftTests: MHMapViewIntegrationTest {
    // Create snapshot options
    private class func snapshotterOptions(size: CGSize) -> MHMapSnapshotOptions {
        let camera = MHMapCamera()

        let options = MHMapSnapshotOptions(styleURL: MHStyle.predefinedStyle("Hybrid").url, camera: camera, size: size)

        let sw = CLLocationCoordinate2D(latitude: 52.3, longitude: 13.0)
        let ne = CLLocationCoordinate2D(latitude: 52.5, longitude: 13.2)
        options.coordinateBounds = MHCoordinateBounds(sw: sw, ne: ne)
    ...
    override public func setUp() {
        super.setUp()
        MHSettings.use(MHWellKnownTileServer.mapTiler)
    }
    ...
    func testCapturingSnapshotterInSnapshotCompletionLOCKED() {
        ...
        let options = MHMapSnapshotterSwiftTests.snapshotterOptions(size: mapView.bounds.size)
