...
@interface MBGLIntegrationTests : MHMapViewIntegrationTest
@end
...
- (void)testOpenGLLayerDoesNotLeakWhenCreatedAndDestroyedWithoutAddingToStyle {
    MHCustomStyleLayer *layer = [[MHCustomStyleLayer alloc] initWithIdentifier:@"gl-layer"];
...
- (void)testReusingOpenGLLayerIdentifier {
    __weak MHCustomStyleLayer *weakLayer2;

    @autoreleasepool {
        MHCustomStyleLayer *layer1 = [[MHCustomStyleLayer alloc] initWithIdentifier:@"gl-layer"];
        [self.style insertLayer:layer1 atIndex:0];
        [self waitForMapViewToBeRendered];
        [self.style removeLayer:layer1];

        MHCustomStyleLayer *layer2 = [[MHCustomStyleLayer alloc] initWithIdentifier:@"gl-layer"];
        weakLayer2 = layer2;
...
- (void)testOpenGLLayerDoesNotLeakWhenStyleChanged {
    __weak MHCustomStyleLayer *weakLayer;
    ...
    MHStyleLayer *layer2 = [self.mapView.style layerWithIdentifier:@"gl-layer"];
    ...
    MHStyleLayer *layer3 = [self.mapView.style layerWithIdentifier:@"gl-layer"];
    XCTAssertNil(layer3);
}
...
- (void)testOpenGLLayerDoesNotLeakWhenMapViewDeallocs {
    ...
    @autoreleasepool {
        ...
        NSURL *styleURL = [[NSBundle bundleForClass:[self class]] URLForResource:@"one-liner" withExtension:@"json"];
        MHMapView *mapView2 = [[MHMapView alloc] initWithFrame:UIScreen.mainScreen.bounds styleURL:styleURL];
...
- (void)testMHMapViewImplHasCorrectSize {
    NSURL *styleURL = [[NSBundle bundleForClass:[self class]] URLForResource:@"one-liner" withExtension:@"json"];
    self.styleLoadingExpectation = [self expectationWithDescription:@"Map view should finish loading style."];
    [self.mapView setStyleURL:styleURL];
    ...
    MHMapViewImpl *mapViewImpl = [self.mapView viewImpl];
    ...
    MHPointCollectionFeature *points = [MHPointCollectionFeature pointCollectionWithCoordinates:coordinates count:sizeof(coordinates)/sizeof(coordinates[0])];
    MHShapeSource *source = [[MHShapeSource alloc] initWithIdentifier:@"heatmap" shape:points options:nil];
    [self.style addSource:source];

    MHHeatmapStyleLayer *heatmapLayer = [[MHHeatmapStyleLayer alloc] initWithIdentifier:@"lineLayer" source:source];
