...
#import "MHTestLocationManager.h"
...
@interface MHTestCalloutView : MHCompactCalloutView
...
@interface MHAnnotationViewIntegrationTests : MHMapViewIntegrationTest
@end

@implementation MHAnnotationViewIntegrationTests
...
- (void)internalTestOffscreenSelectionTitle:(NSString*)title withTestData:(PanTestData)test animateSelection:(BOOL)animateSelection {
    ...
    NSString * const MHTestAnnotationReuseIdentifer = @"MHTestAnnotationReuseIdentifer";
    ...
    self.viewForAnnotation = ^MHAnnotationView*(MHMapView *view, id<MHAnnotation> annotation) {
        ...
        // No dequeue
        MHAnnotationView *annotationView = [[MHAnnotationView alloc] initWithAnnotation:annotation reuseIdentifier:MHTestAnnotationReuseIdentifer];
    ...
    self.mapViewAnnotationCanShowCalloutForAnnotation = ^BOOL(MHMapView *mapView, id<MHAnnotation> annotation) {
        return showsCallout;
    };
    
    self.mapViewCalloutViewForAnnotation = ^id<MHCalloutView>(MHMapView *mapView, id<MHAnnotation> annotation) {
        if (!showsCallout)
            return nil;
        
        MHTestCalloutView *calloutView = [[MHTestCalloutView alloc] init];
    ...
    // Animated selection takes MHAnimationDuration (0.3 seconds), so wait a little
    ...
    if (expectMapToHavePanned) {
        ...
        if (showsCallout) {
            ...
            // if you need this NOT to fail the tests, consider replacing with MHTestWarning
...
- (void)internalTestSelectingAnnotationWithCenterOffsetWithOffset:(CGVector)offset edgeInsets:(UIEdgeInsets)edgeInsets {
    ...
    MHAnnotationTag tagAtPoint = [self.mapView annotationTagAtPoint:tapPoint persistingResults:YES];
...
- (void)testUserLocationWithOffsetAnchorPoint {
    ...
    MHTestLocationManager *locationManager = [[MHTestLocationManager alloc] init];
    self.mapView.locationManager = locationManager;

    [self.mapView setUserTrackingMode:MHUserTrackingModeFollow animated:NO completionHandler:nil];
    ...
    [self.mapView setUserTrackingMode:MHUserTrackingModeNone animated:NO completionHandler:nil];
    
    CGPoint offset = CGPointMake(20, 20);
    
    self.mapViewUserLocationAnchorPoint = ^CGPoint (MHMapView *mapView) {
        return offset;;
    };
...
- (void)testSelectingAnnotationWhenMapIsRotated {
    ...
    MHPointAnnotation *annotation = annotations.firstObject;
    ...
    for (NSInteger iter = 0; iter < 10; iter++ ) {
        ...
        [XCTContext runActivityNamed:activityTitle
                               block:^(id<XCTActivity>  _Nonnull activity) {
                                   ...
                                   MHMapCamera *toCamera = [self.mapView cameraByRotatingToDirection:angle aroundAnchorPoint:anchor];
...
- (void)testSelectingAnnotationWhenMapIsScaled {
    ...
    for (NSInteger iter = 0; iter < 10; iter++ ) {
        ...
        [XCTContext runActivityNamed:activityTitle
                               block:^(id<XCTActivity>  _Nonnull activity) {
                                   MHMapCamera *toCamera = [self.mapView cameraByZoomingToZoomLevel:zoom aroundAnchorPoint:anchor];
                                   [self internalTestSelecting:annotation withCamera:toCamera];
                               }];
    }
}
    ...
    for (NSInteger iter = 0; iter < 10; iter++ ) {
        ...
        [XCTContext runActivityNamed:activityTitle
                               block:^(id<XCTActivity>  _Nonnull activity)
         {
             ...
             MHMapCamera *toCamera = [self.mapView cameraForCameraOptions:currentCameraOptions];
...
- (void)internalTestShowingAnnotationsThenSelectingAnimated:(BOOL)animated edgePadding:(UIEdgeInsets)edgeInsets contentInsets:(UIEdgeInsets)contentInsets {
    ...
    for (MHPointAnnotation *point in annotations) {
...
- (NSArray*)internalAddAnnotationsAtCoordinates:(CLLocationCoordinate2D*)coordinates
{
    ...
    [XCTContext runActivityNamed:@"Map setup"
                           block:^(id<XCTActivity>  _Nonnull activity)
     {
         ...
         self.viewForAnnotation = ^MHAnnotationView*(MHMapView *view, id<MHAnnotation> annotation2) {
             
             if (![annotation2 isKindOfClass:[MHPointAnnotation class]]) {
                 return nil;
             }
             
             // No dequeue
             MHAnnotationView *annotationView = [[MHAnnotationView alloc] initWithAnnotation:annotation2 reuseIdentifier:MHTestAnnotationReuseIdentifer];
             annotationView.bounds             = (CGRect){ .origin = CGPointZero, .size = annotationSize };
             annotationView.backgroundColor    = UIColor.redColor;
             annotationView.enabled            = YES;
...
- (void)internalTestSelecting:(MHPointAnnotation*)point withCamera:(MHMapCamera*)camera {
...
- (void)internalSelectDeselectAnnotation:(MHPointAnnotation*)point {
    [XCTContext runActivityNamed:[NSString stringWithFormat:@"Select annotation: %@", point]
                           block:^(id<XCTActivity>  _Nonnull activity)
     {
         CGPoint annotationPoint = [self.mapView convertCoordinate:point.coordinate toPointToView:self.mapView];
         
         MHAnnotationTag tagAtPoint = [self.mapView annotationTagAtPoint:annotationPoint persistingResults:YES];
         if (tagAtPoint != UINT32_MAX)
         {
             id <MHAnnotation> annotation = [self.mapView annotationWithTag:tagAtPoint];
