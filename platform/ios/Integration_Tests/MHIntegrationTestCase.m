...
@implementation MHIntegrationTestCase
...
+ (XCTestSuite*)defaultTestSuite {
    ...
    for (XCTest *test in tests) {
        ...
        if ([test.name containsString:@"LOCKED"]) {
            if (!apiKey) {
                printf("warning: MH_API_KEY env var is required for test '%s' - skipping.\n", test.name.UTF8String);
                continue;
            }
        }
...
- (void)setUp {
    ...
    if ([self.name containsString:@"LOCKED"]) {
        apiKey = [[NSProcessInfo processInfo] environment][@"MH_API_KEY"];

        if (!apiKey) {
            printf("warning: MH_API_KEY env var is required for test '%s' - trying anyway.\n", self.name.UTF8String);
        }
    }

    [MHSettings setApiKey:apiKey ?: @"pk.feedcafedeadbeefbadebede"];
}
@end
