...
@interface MHCameraTransitionTests : MHMapViewIntegrationTest
@end

@implementation MHCameraTransitionTests
...
- (void)testSetAndResetNorthWithDispatchAsyncInDelegateMethod {
    ...
    __weak MHCameraTransitionTests *weakself = self;

    self.regionDidChange = ^(MHMapView *mapView, MHCameraChangeReason reason, BOOL animated) {

        MHCameraTransitionTests *strongSelf = weakself;

        if (!strongSelf) return;

        [expectation fulfill];

        MHTestAssert(strongSelf, mapView.userTrackingMode != MHUserTrackingModeFollowWithHeading);
...
- (void)testInterruptingAndResetNorthOnlyOnceInIsChanging {
    ...
    self.regionDidChange = ^(MHMapView *mapView, MHCameraChangeReason reason, BOOL animated) {
        ...
        MHTestAssert(strongSelf, startedReset);

        if (finishedReset) {
            MHTestAssert(strongSelf, !(reason & MHCameraChangeReasonTransitionCancelled));
            [expectation fulfill];
        }
        else {
            MHTestAssert(strongSelf, reason & MHCameraChangeReasonTransitionCancelled);
...
- (void)testSetCenterCancelsTransitions {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 0.15 * NSEC_PER_SEC),
                   ...
                   ^{
                       ...
                       MHTestAssertEqualWithAccuracy(strongSelf,
                                                      dc.latitude,
    ...
    self.regionDidChange = ^(MHMapView *mapView, MHCameraChangeReason reason, BOOL animated) {
        ...
        switch(delegateCallCount) {
            case 0:
            {
                ...
                // Should take MHAnimationDuration seconds (0.3s)
...
- (void)testFlyToCameraInDelegateMethod {
    ...
    double altitude = MHAltitudeForZoomLevel(zoomLevel, 0.0, target.latitude, self.mapView.frame.size);
    ...
    self.regionDidChange = ^(MHMapView *mapView, MHCameraChangeReason reason, BOOL animated) {
        ...
        switch(delegateCallCount) {
            case 0:
            {
                ...
                // Now set another coordinate.
                MHMapCamera *camera = [MHMapCamera cameraLookingAtCenterCoordinate:target2
                                                                            altitude:altitude
                                                                               pitch:0.0
                                                                             heading:0.0];
                ...
                [mapView flyToCamera:camera completionHandler:^{
                    MHTestAssert(strongSelf, !runloop, @"Completion block should be called after delegate method");
                    [expectation fulfill];
    ...
    // Should take MHAnimationDuration
