...
- (instancetype)initWithFrame:(CGRect)frame options:(MHMapOptions *)options
{
    ...
    {
        MHLogInfo(@"Starting %@ initialization.", NSStringFromClass([self class]));
        MHLogDebug(@"Initializing frame: %@ with options", NSStringFromRect(frame));
...
- (void)addOverlays:(NSArray<id <MHOverlay>> *)overlays
{
    ...
    for (id <MHOverlay> overlay in overlays) {
        NSAssert([overlay conformsToProtocol:@protocol(MHOverlay)], @"Overlay does not conform to MHOverlay.h");
    }
#endif
