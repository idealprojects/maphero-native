...
BOOL MHEdgeInsetsIsZero(NSEdgeInsets edgeInsets) {
    return edgeInsets.left == 0 && edgeInsets.top == 0 && edgeInsets.right == 0 && edgeInsets.bottom == 0;
}

@implementation NSImage (MHAdditions)

- (nullable instancetype)initWithMHPremultipliedImage:(mbgl::PremultipliedImage&&)src {
    CGImageRef image = CGImageCreateWithMHPremultipliedImage(std::move(src));
    if (!image) {
        return nil;
    }
...
- (mbgl::PremultipliedImage)mgl_premultipliedImage {
    ...
    return MHPremultipliedImageFromCGImage(ref);
}
