...
Some vector tiles servers require a API key. In the project editor, select the application target, then go to the Info tab. Under the Info section, Under the “Custom macOS Application Target Properties” section, set `MHApiKey` to your api key.
...
2. In the Identity inspector, set the view’s custom class to `MHMapView`.
3. MHMapView needs to be layer-backed:
