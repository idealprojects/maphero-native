...
#if TARGET_OS_IPHONE
#import "UIColor+MHAdditions.h"
#else
#import "NSColor+MHAdditions.h"
#endif
#import "MHAttributedExpression.h"

#define MHAssertEqualValues(actual, expected, ...) \
...
#define MHAssertEqualValuesWithAccuracy(actual, expected, accuracy, ...) \
    XCTAssertTrue(actual.is<__typeof__(expected)>()); \
    if (actual.is<__typeof__(expected)>()) { \
        XCTAssertEqualWithAccuracy(actual.get<__typeof__(expected)>(), expected, accuracy, __VA_ARGS__); \
    }

#define MHConstantExpression(constant) \
    [NSExpression expressionForConstantValue:constant]

#define MHAssertConstantEqualsValue(constant, value, ...) \
    MHAssertEqualValues(MHConstantExpression(constant).mgl_constantMBGLValue, value, __VA_ARGS__);

#define MHAssertConstantEqualsValueWithAccuracy(constant, value, accuracy, ...) \
    MHAssertEqualValuesWithAccuracy(MHConstantExpression(constant).mgl_constantMBGLValue, value, accuracy, __VA_ARGS__);

using namespace std::string_literals;

@interface MHExpressionTests : XCTestCase

@end

@implementation MHExpressionTests
...
// MARK: - Objective-C Example Code for MH Expressions
...
- (void)testStringValuation {
    MHAssertConstantEqualsValue(@"bar", "bar"s, @"NSString should convert to std::string.");
    MHAssertConstantEqualsValue(@"🆔🆗🇦🇶", "🆔🆗🇦🇶"s, @"NSString with non-ASCII characters should convert losslessly to std::string.");
}

- (void)testColorValuation {
    MHAssertConstantEqualsValue([MHColor redColor], "rgba(255,0,0,1)"s, @"MHColor should convert to std::string containing CSS color string.");
}

- (void)testBooleanValuation {
    MHAssertConstantEqualsValue(@NO, false, @"Boolean NSNumber should convert to bool.");
    MHAssertConstantEqualsValue(@YES, true, @"Boolean NSNumber should convert to bool.");
}

- (void)testDoubleValuation
{
    MHAssertConstantEqualsValue(@DBL_MIN, DBL_MIN, @"Double NSNumber should convert to double.");
    MHAssertConstantEqualsValue(@DBL_MAX, DBL_MAX, @"Double NSNumber should convert to double.");
}
...
- (void)testFloatValuation {
    ...
    MHAssertConstantEqualsValue(@-1.0f, -1.0, @"Float NSNumber should convert to double.");
    MHAssertConstantEqualsValue(@1.0f, 1.0, @"Float NSNumber should convert to double.");
    MHAssertConstantEqualsValueWithAccuracy(@-23.232342f, -23.232342, 0.000001, @"Float NSNumber should convert to double.");
    MHAssertConstantEqualsValueWithAccuracy(@23.232342f, 23.232342, 0.000001, @"Float NSNumber should convert to double.");
    MHAssertConstantEqualsValueWithAccuracy(@-FLT_MAX, static_cast<double>(-FLT_MAX), 0.000001, @"Float NSNumber should convert to double.");
    MHAssertConstantEqualsValueWithAccuracy(@FLT_MAX, static_cast<double>(FLT_MAX), 0.000001, @"Float NSNumber should convert to double.");
}

- (void)testIntegerValuation {
    // Negative integers should always come back as int64_t per mbgl::Value definition.
    MHAssertConstantEqualsValue(@SHRT_MIN, static_cast<int64_t>(SHRT_MIN), @"Negative short NSNumber should convert to int64_t.");
    MHAssertConstantEqualsValue(@INT_MIN, static_cast<int64_t>(INT_MIN), @"Negative int NSNumber should convert to int64_t.");
    MHAssertConstantEqualsValue(@LONG_MIN, static_cast<int64_t>(LONG_MIN), @"Negative long NSNumber should convert to int64_t.");
    MHAssertConstantEqualsValue(@LLONG_MIN, static_cast<int64_t>(LLONG_MIN), @"Negative long long NSNumber should convert to int64_t.");
    MHAssertConstantEqualsValue(@NSIntegerMin, static_cast<int64_t>(NSIntegerMin), @"Negative NSInteger NSNumber should convert to int64_t.");

    // Positive integers should always come back as uint64_t per mbgl::Value definition.
    MHAssertConstantEqualsValue(@SHRT_MAX, static_cast<uint64_t>(SHRT_MAX), @"Positive short NSNumber should convert to uint64_t.");
    MHAssertConstantEqualsValue(@INT_MAX, static_cast<uint64_t>(INT_MAX), @"Positive int NSNumber should convert to uint64_t.");
    MHAssertConstantEqualsValue(@LONG_MAX, static_cast<uint64_t>(LONG_MAX), @"Positive long NSNumber should convert to uint64_t.");
    MHAssertConstantEqualsValue(@LLONG_MAX, static_cast<uint64_t>(LLONG_MAX), @"Positive long long NSNumber should convert to uint64_t.");
    MHAssertConstantEqualsValue(@NSIntegerMax, static_cast<uint64_t>(NSIntegerMax), @"Positive NSInteger NSNumber should convert to uint64_t.");
}
...
- (void)testUnsignedIntegerValuation {
    ...
    MHAssertConstantEqualsValue(@(static_cast<unsigned short>(0)), static_cast<uint64_t>(0), @"Unsigned short NSNumber should convert to uint64_t.");
    MHAssertConstantEqualsValue(@0u, static_cast<uint64_t>(0), @"Unsigned int NSNumber should convert to uint64_t.");
    MHAssertConstantEqualsValue(@0UL, static_cast<uint64_t>(0), @"Unsigned long NSNumber should convert to uint64_t.");
    MHAssertConstantEqualsValue(@0ULL, static_cast<uint64_t>(0), @"Unsigned long long NSNumber should convert to uint64_t.");
    MHAssertConstantEqualsValue(@(static_cast<NSUInteger>(0)), static_cast<uint64_t>(0), @"Unsigned NSUInteger NSNumber should convert to uint64_t.");

    // Positive integers should always come back as uint64_t per mbgl::Value definition.
    // We use the unsigned long long value because it can store the highest number on
    // both 32- and 64-bit and won't overflow.
    MHAssertConstantEqualsValue(@USHRT_MAX, static_cast<uint64_t>(USHRT_MAX), @"Unsigned short NSNumber should convert to uint64_t.");
    MHAssertConstantEqualsValue(@UINT_MAX, static_cast<uint64_t>(UINT_MAX), @"Unsigned int NSNumber should convert to uint64_t.");
    MHAssertConstantEqualsValue(@ULONG_MAX, static_cast<uint64_t>(ULONG_MAX), @"Unsigned long NSNumber should convert to uint64_t.");
    MHAssertConstantEqualsValue(@ULLONG_MAX, static_cast<uint64_t>(ULLONG_MAX), @"Unsigned long long NSNumber should convert to uint64_t.");
    MHAssertConstantEqualsValue(@NSUIntegerMax, static_cast<uint64_t>(NSUIntegerMax), @"Unsigned NSUInteger NSNumber should convert to uint64_t.");
}

- (void)testNullValuation {
    mbgl::NullValue nullValue;
    MHAssertConstantEqualsValue([NSNull null], nullValue, @"NSNull should convert to mbgl::NullValue.");
}
...
- (void)testVariableExpressionObject {
    {
        ...
        XCTAssertEqualObjects([NSExpression expressionWithMHJSONObject:@[@"zoom"]], expression);
    ...
    }
    {
        ...
        XCTAssertEqualObjects([NSExpression expressionWithMHJSONObject:@[@"heatmap-density"]], expression);
    ...
    }
    {
        ...
        XCTAssertEqualObjects([NSExpression expressionWithMHJSONObject:@[@"line-progress"]], expression);
    ...
    }
    {
        ...
        XCTAssertEqualObjects([NSExpression expressionWithMHJSONObject:@[@"accumulated"]], expression);
    }

    {
        ...
        XCTAssertEqualObjects([NSExpression expressionWithMHJSONObject:@[@"geometry-type"]], expression);
    }
    {
        ...
        XCTAssertEqualObjects([NSExpression expressionWithMHJSONObject:@[@"id"]], expression);
    }
    {
        ...
        XCTAssertEqualObjects([NSExpression expressionWithMHJSONObject:@[@"properties"]], expression);
    }
    {
    ...
    {
        NSDictionary *context = @{@"loremIpsum": MHConstantExpression(@"Lorem ipsum dolor sit amet")};
        #if TARGET_OS_IPHONE
        XCTExpectFailure(@"Awaiting unit test refactoring for https://github.com/maplibre/maplibre-native/issues/331");
        #endif
        NSExpression *expression = [NSExpression expressionWithFormat:@"MH_LET('loremIpsum', 'Lorem ipsum dolor sit amet', uppercase($loremIpsum))", context];
...
- (void)testConstantValueExpressionObject {
    {
        ...
        XCTAssertEqualObjects([NSExpression expressionWithMHJSONObject:[NSNull null]], expression);
    ...
    }
    {
        ...
        XCTAssertEqualObjects([NSExpression expressionWithMHJSONObject:@1], expression);
    ...
    }
    {
        ...
        XCTAssertEqualObjects([NSExpression expressionWithMHJSONObject:@YES], expression);
    ...
    }
    {
        ...
        XCTAssertEqualObjects([[NSExpression expressionWithMHJSONObject:jsonExpression].collection valueForKeyPath:@"constantValue"], jsonExpression.lastObject);
    ...
    {
        MHColor *color = [MHColor mgl_colorWithColor:{ 255.0/255, 239.0/255, 213.0/255, 1 }]; // papayawhip
    ...
    }
    {
        ...
        MHColor *color = [MHColor mgl_colorWithColor:{ red, green, blue, alpha }]; // papayawhip
        NSExpression *expression = [NSExpression expressionForConstantValue:color];
    ...
    }
    {
        // Transform color components to non-premultiplied values
        NSArray *jsonExpression = @[@"rgba", @255.0, @239.0, @213.0, @0.5];
        NSExpression *papayawhipExpression = [NSExpression expressionWithMHJSONObject:jsonExpression];
        MHColor *papayaWhipColor = [papayawhipExpression expressionValueWithObject:nil context:nil];
...
- (void)testKeyPathExpressionObject {
    ...
    {
        NSExpression *expression = [NSExpression expressionWithFormat:@"%@.population", @{@"population": MHConstantExpression(@12000)}];
        NSArray *jsonExpression = @[@"get", @"population", @[@"literal", @{@"population": @12000}]];
        XCTAssertEqualObjects(expression.mgl_jsonExpressionObject, jsonExpression);
        XCTAssertEqualObjects([NSExpression expressionWithMHJSONObject:jsonExpression], expression);
    }
    {
        NSExpression *expression = [NSExpression expressionWithFormat:@"%@.uppercase('population')", @{@"POPULATION": MHConstantExpression(@12000)}];
...
- (void)testArithmeticExpressionObject {
    NSArray *arguments = @[MHConstantExpression(@1), MHConstantExpression(@1)];
    {
    ...
    }
    {
        ...
        NSExpression *expression = [NSExpression expressionForFunction:@"sum:" arguments:@[[NSExpression expressionForAggregate:@[MHConstantExpression(@1), MHConstantExpression(@1), MHConstantExpression(@2)]]]];
    ...
    {
        NSExpression *expression = [NSExpression expressionForFunction:@"sum:" arguments:@[MHConstantExpression(@1), MHConstantExpression(@1), MHConstantExpression(@2)]];
        NSArray *jsonExpression = @[@"+", @1, @1, @2];
        XCTAssertEqualObjects(expression.mgl_jsonExpressionObject, jsonExpression);

        // - [NSExpression expressionWithMHJSONObject:] creates an expression with an aggregate expression as an argument. This is not equal to an expression with an array of expressions as an argument. For testing purposes, we will compare their operands and arrays of expressions.
        NSExpression *aggregateExpression = [NSExpression expressionWithMHJSONObject:jsonExpression];
    ...
    {
        NSArray *threeArguments = @[MHConstantExpression(@1), MHConstantExpression(@1), MHConstantExpression(@1)];
        ...
        XCTAssertEqualObjects([NSExpression expressionWithMHJSONObject:jsonExpression], [NSExpression expressionWithFormat:@"1 + 1 + 1"]);
    }
    {
    ...
    {
        NSExpression *expression = [NSExpression expressionForFunction:@"ceiling:" arguments:@[MHConstantExpression(@1.5)]];
    ...
    {
        NSExpression *expression = [NSExpression expressionForFunction:@"ceiling:" arguments:@[MHConstantExpression(@2)]];
    ...
    {
        NSExpression *expression = [NSExpression expressionForFunction:@"trunc:" arguments:@[MHConstantExpression(@1.5)]];
    ...
    {
        NSExpression *expression = [NSExpression expressionForFunction:@"abs:" arguments:@[MHConstantExpression(@2)]];
    ...
    {
        NSExpression *expression = [NSExpression expressionForFunction:@"floor:" arguments:@[MHConstantExpression(@1.5)]];
    ...
    {
        NSExpression *expression = [NSExpression expressionForFunction:@"floor:" arguments:@[MHConstantExpression(@2)]];
    ...
    }
    {
        ...
        XCTExpectFailure(@"Awaiting unit test refactoring for https://github.com/maplibre/maplibre-native/issues/331");
        #endif
        NSExpression *expression = [NSExpression expressionForFunction:@"mgl_round:" arguments:@[MHConstantExpression(@1.5)]];
    ...
    {
        NSExpression *expression = [NSExpression expressionForFunction:@"mgl_round:" arguments:@[MHConstantExpression(@2.5)]];
...
- (void)testTrigonometricExpressionObject {
    ...
    }
    {
        ...
        XCTExpectFailure(@"Awaiting unit test refactoring for https://github.com/maplibre/maplibre-native/issues/331");
        #endif
        NSExpression *expression = [NSExpression expressionForFunction:@"mgl_log2:" arguments:@[MHConstantExpression(@1024)]];
    ...
    {
        NSExpression *expression = [NSExpression expressionForFunction:@"exp:" arguments:@[MHConstantExpression(@0)]];
    ...
    {
        NSExpression *expression = [NSExpression expressionForFunction:@"mgl_acos:" arguments:@[MHConstantExpression(@1)]];
    ...
    {
        NSExpression *expression = [NSExpression expressionForFunction:@"mgl_cos:" arguments:@[MHConstantExpression(@0)]];
    ...
    {
        NSExpression *expression = [NSExpression expressionForFunction:@"mgl_asin:" arguments:@[MHConstantExpression(@0)]];
    ...
    {
        NSExpression *expression = [NSExpression expressionForFunction:@"mgl_sin:" arguments:@[MHConstantExpression(@0)]];
    ...
    {
        NSExpression *expression = [NSExpression expressionForFunction:@"mgl_atan:" arguments:@[MHConstantExpression(@20)]];
    ...
    {
        NSExpression *expression = [NSExpression expressionForFunction:@"mgl_tan:" arguments:@[MHConstantExpression(@0)]];
...
- (void)testShapeDistanceExpressionObject {
    {
        MHPointAnnotation *point = [[MHPointAnnotation alloc] init];
        point.coordinate = CLLocationCoordinate2DMake(1, -1);
        #if TARGET_OS_IPHONE
        XCTExpectFailure(@"Awaiting unit test refactoring for https://github.com/maplibre/maplibre-native/issues/331");
        #endif
        NSExpression *expression = [NSExpression expressionForFunction:@"mgl_distanceFrom:" arguments:@[MHConstantExpression(point)]];
...
- (void)testStringFormattingExpressionObject {
    NSArray *arguments = @[MHConstantExpression(@"MacDonald")];
    {
        ...
        XCTAssertEqualObjects([NSExpression expressionWithMHJSONObject:jsonExpression], aftermarketExpression);
    }
    {
...
- (void)testTypeConversionExpressionObject {
    ...
    }
    {
        ...
        XCTAssertEqualObjects([NSExpression expressionWithMHJSONObject:jsonExpression],
    ...
    }
    {
        ...
        XCTExpectFailure(@"Awaiting unit test refactoring for https://github.com/maplibre/maplibre-native/issues/331");
        #endif
        NSExpression *expression = [NSExpression expressionWithFormat:@"MH_FUNCTION('to-color', x, y, z)"];
    ...
    {
        NSExpression *expression = [NSExpression expressionWithFormat:@"CAST(noindex(%@), 'NSArray')", MHConstantExpression(MHColor.blueColor)];
...
- (void)testInterpolationExpressionObject {
    {
        NSDictionary *stops = @{@0: MHConstantExpression(@100), @10: MHConstantExpression(@200)};
        XCTExpectFailure(@"Awaiting unit test refactoring for https://github.com/maplibre/maplibre-native/issues/331");
    ...
    {
        NSDictionary *stops = @{@1: MHConstantExpression(@2), @3: MHConstantExpression(@6)};
    ...
    {
        NSDictionary *stops = @{@0: MHConstantExpression(@0), @100: MHConstantExpression(@100)};
    ...
    {
        NSDictionary *stops = @{@0: MHConstantExpression(@111), @1: MHConstantExpression(@1111)};
...
- (void)testMatchExpressionObject {
    {
        ...
        XCTExpectFailure(@"Awaiting unit test refactoring for https://github.com/maplibre/maplibre-native/issues/331");
        #endif
        NSExpression *expression = [NSExpression expressionWithFormat:@"MH_MATCH(2 - 1,  %@, %@, %@, %@, 'default')", MHConstantExpression(@1),
                                    MHConstantExpression(@"one"),
                                    MHConstantExpression(@0),
                                    MHConstantExpression(@"zero")];
        NSExpression *predicate = [NSExpression expressionWithFormat:@"2 - 1"];
        NSExpression *compatibilityExpression = [NSExpression expressionWithFormat:@"FUNCTION(%@, 'mgl_match:', %@)", predicate, @[MHConstantExpression(@1),
                                                 MHConstantExpression(@"one"),
                                                 MHConstantExpression(@0),
                                                 MHConstantExpression(@"zero"),
                                                 MHConstantExpression(@"default")]];
    ...
    {
        NSExpression *expression = [NSExpression expressionWithFormat:@"MH_MATCH(2 * 1, %@, %@, 'default')", MHConstantExpression(@1), MHConstantExpression(@"one")];
        NSArray *jsonExpression =  @[@"match", @[@"*", @2, @1], @1, @"one", @"default"];
        XCTAssertEqualObjects(expression.mgl_jsonExpressionObject, jsonExpression);
        XCTAssertEqualObjects([NSExpression expressionWithMHJSONObject:jsonExpression], expression);
    }
    {
        NSExpression *expression = [NSExpression expressionWithFormat:@"MH_MATCH(x, {'a', 'A'}, 'Apple', {'b', 'B'}, 'Banana', 'Kumquat')"];
        NSArray *jsonExpression =  @[@"match", @[@"get", @"x"], @[@"a", @"A"], @"Apple", @[@"b", @"B"], @"Banana", @"Kumquat"];
        XCTAssertEqualObjects(expression.mgl_jsonExpressionObject, jsonExpression);
        XCTAssertEqualObjects([NSExpression expressionWithMHJSONObject:jsonExpression], expression);
    }
    {
        NSExpression *expression = [NSExpression expressionWithFormat:@"MH_MATCH(x, %@, 'Apple', %@, 'Banana', 'Kumquat')",
                                    @[@"a", @"A"], @"Bb"];
        NSArray *jsonExpression =  @[@"match", @[@"get", @"x"], @[@"a", @"A"], @"Apple", @"Bb", @"Banana", @"Kumquat"];
        XCTAssertEqualObjects(expression.mgl_jsonExpressionObject, jsonExpression);
        XCTAssertEqualObjects([NSExpression expressionWithMHJSONObject:jsonExpression].description, expression.description);
    }
}
...
- (void)testConditionalExpressionObject {
    ...
    }
    {
        ...
        XCTExpectFailure(@"Awaiting unit test refactoring for https://github.com/maplibre/maplibre-native/issues/331");
        #endif
        NSExpression *expression = [NSExpression expressionWithFormat:@"MH_IF(%@, %@, %@)",
                                    [NSExpression expressionWithFormat:@"%@", [NSPredicate predicateWithFormat:@"1 = 2"]],
                                    MHConstantExpression(@YES),
                                    MHConstantExpression(@NO)];
        NSExpression *compatibilityExpression = [NSExpression expressionWithFormat:@"FUNCTION(%@, 'mgl_if:', %@)", [NSPredicate predicateWithFormat:@"1 = 2"], @[MHConstantExpression(@YES), MHConstantExpression(@NO)]];
        NSArray *jsonExpression = @[@"case", @[@"==", @1, @2], @YES, @NO];
        XCTAssertEqualObjects(expression.mgl_jsonExpressionObject, jsonExpression);
        XCTAssertEqualObjects(compatibilityExpression.mgl_jsonExpressionObject, jsonExpression);
...
- (void)testLookupExpressionObject {
    {
        NSExpression *array = [NSExpression expressionForAggregate:@[MHConstantExpression(@9),
                                                                     MHConstantExpression(@8),
                                                                     MHConstantExpression(@7)]];
        NSExpression *expression = [NSExpression expressionForFunction:@"objectFrom:withIndex:"
                                                             arguments:@[array, MHConstantExpression(@"FIRST")]];
    ...
    }
    {
        NSExpression *array = [NSExpression expressionForAggregate:@[MHConstantExpression(@9),
                                                                     ...
                                                                     MHConstantExpression(@7)]];
        NSExpression *expression = [NSExpression expressionForFunction:@"objectFrom:withIndex:"
                                                             arguments:@[array, MHConstantExpression(@"LAST")]];
    ...
    }
    {
        NSExpression *array = [NSExpression expressionForAggregate:@[MHConstantExpression(@9),
                                                                     ...
                                                                     MHConstantExpression(@7)]];
        NSExpression *expression = [NSExpression expressionForFunction:@"objectFrom:withIndex:"
                                                             arguments:@[array, MHConstantExpression(@"SIZE")]];
    ...
    }
    {
        NSExpression *array = [NSExpression expressionForAggregate:@[MHConstantExpression(@9),
                                                                     ...
                                                                     MHConstantExpression(@7)]];
        NSExpression *expression = [NSExpression expressionForFunction:@"objectFrom:withIndex:"
                                                             arguments:@[array, MHConstantExpression(@1)]];
    ...
    {
        NSExpression *expression = [NSExpression expressionForFunction:@"mgl_does:have:"
                                                             arguments:@[MHConstantExpression(@{@"x": MHConstantExpression(@0)}),
                                                                         MHConstantExpression(@"x")]];
...
- (void)testFormatExpressionObject {
    {
        MHAttributedExpression *attribute1 = [MHAttributedExpression attributedExpression:[NSExpression expressionForConstantValue:@"foo"]
                                                                                  fontNames:nil
                                                                                  fontScale:@(1.2)];
        MHAttributedExpression *attribute2 = [MHAttributedExpression attributedExpression:[NSExpression expressionForConstantValue:@"biz"]
                                                                                  fontNames:nil
                                                                                  fontScale:@(1.0)];
        MHAttributedExpression *attribute3 = [MHAttributedExpression attributedExpression:[NSExpression expressionForConstantValue:@"bar"]
                                                                                  fontNames:nil
                                                                                  fontScale:@(0.8)];
        MHAttributedExpression *attribute4 = [MHAttributedExpression attributedExpression:[NSExpression expressionForConstantValue:@"\r"]
                                                                                  fontNames:@[]
                                                                                  fontScale:nil];
        #if TARGET_OS_IPHONE
        XCTExpectFailure(@"Awaiting unit test refactoring for https://github.com/maplibre/maplibre-native/issues/331");
        ...
        NSExpression *expression = [NSExpression expressionWithFormat:@"mgl_attributed:(%@, %@, %@, %@)",
                                    MHConstantExpression(attribute1),
                                    MHConstantExpression(attribute4),
                                    MHConstantExpression(attribute2),
                                    MHConstantExpression(attribute3)];
    ...
    }
    {
        ...
        MHAttributedExpression *attribute3 = [MHAttributedExpression attributedExpression:[NSExpression expressionForConstantValue:@"bar"]
                                                                                  fontNames:nil
                                                                                  fontScale:@(0.8)];
        MHAttributedExpression *attribute4 = [MHAttributedExpression attributedExpression:[NSExpression expressionForConstantValue:@"\n"]
                                                                                  fontNames:@[]
                                                                                  fontScale:nil];
        NSExpression *expression = [NSExpression expressionWithFormat:@"mgl_attributed:(%@, %@, %@, %@)",
                                    MHConstantExpression(attribute1),
    ...
    }
    {
        MHAttributedExpression *attribute1 = [MHAttributedExpression attributedExpression:[NSExpression expressionForConstantValue:@"foo"]
                                                                                  fontNames:nil
                                                                                   fontScale:@(1.2)];
        NSExpression *expression = [NSExpression expressionWithFormat:@"mgl_attributed:(%@)", MHConstantExpression(attribute1)];
        
        NSExpression *compatibilityExpression = [NSExpression expressionForFunction:@"mgl_attributed:" arguments:@[MHConstantExpression(attribute1)]];
        NSArray *jsonExpression = @[@"format", @"foo", @{@"font-scale": @1.2}];
    ...
    {
        MHAttributedExpression *attribute1 = [[MHAttributedExpression alloc] initWithExpression:[NSExpression expressionForConstantValue:@"foo"]
                                                                                       attributes:@{ MHFontScaleAttribute: MHConstantExpression(@(1.2)),
                                                                                                     MHFontColorAttribute: MHConstantExpression(@"yellow") }] ;
        NSExpression *expression = [NSExpression expressionWithFormat:@"mgl_attributed:(%@)", MHConstantExpression(attribute1)];
    ...
    {
        NSExpression *fontNames = [NSExpression expressionForAggregate:@[ MHConstantExpression(@"DIN Offc Pro Bold"), MHConstantExpression(@"Arial Unicode MS Bold") ]];
        MHAttributedExpression *attribute1 = [[MHAttributedExpression alloc] initWithExpression:[NSExpression expressionForConstantValue:@"foo"]
                                                                                       attributes:@{ MHFontScaleAttribute: MHConstantExpression(@(1.2)),
                                                                                                     MHFontColorAttribute: MHConstantExpression(@"yellow"),
                                                                                                     MHFontNamesAttribute: fontNames
                                                                                                     }] ;
        NSExpression *expression = [NSExpression expressionWithFormat:@"mgl_attributed:(%@)", MHConstantExpression(attribute1)];
    ...
    }
    {
        ...
        MHAttributedExpression *attribute1 = [[MHAttributedExpression alloc] initWithExpression:[NSExpression expressionForConstantValue:@"foo"]
                                                                                       attributes:@{ MHFontScaleAttribute: MHConstantExpression(@(1.2)),
                                                                                                     MHFontColorAttribute: MHConstantExpression([MHColor redColor]),
                                                                                                     MHFontNamesAttribute: fontNames
                                                                                                     }] ;
        NSExpression *expression = [NSExpression expressionWithFormat:@"mgl_attributed:(%@)", MHConstantExpression(attribute1)];
    ...
    {
        NSExpression *fontNames = [NSExpression expressionForAggregate:@[ MHConstantExpression(@"DIN Offc Pro Bold"), MHConstantExpression(@"Arial Unicode MS Bold") ]];
        MHAttributedExpression *attribute1 = [[MHAttributedExpression alloc] initWithExpression:[NSExpression expressionWithFormat:@"CAST(x, 'NSString')"]
                                                                                       attributes:@{ MHFontScaleAttribute: MHConstantExpression(@(1.2)),
                                                                                                     MHFontColorAttribute: MHConstantExpression([MHColor redColor]),
                                                                                                     MHFontNamesAttribute: fontNames
                                                                                                     }] ;
    ...
    }
    {
        ...
        MHAttributedExpression *attribute4 = [MHAttributedExpression attributedExpression:[NSExpression expressionForConstantValue:@"\n"]
                                                                                  fontNames:@[]
                                                                                  fontScale:nil];
        NSExpression *expression = [NSExpression mgl_expressionForAttributedExpressions:@[MHConstantExpression(attribute1),
                                                                                          MHConstantExpression(attribute4),
                                                                                          MHConstantExpression(attribute2),
                                                                                          MHConstantExpression(attribute3)]];
        NSArray *jsonExpression = @[@"format", @"foo", @{@"font-scale": @1.2}, @"\n", @{}, @"biz", @{@"font-scale": @1.0}, @"bar", @{@"font-scale": @0.8}];
...
- (void)testGenericExpressionObject {
    {
        ...
        XCTExpectFailure(@"Awaiting unit test refactoring for https://github.com/maplibre/maplibre-native/issues/331");
        #endif
        NSExpression *expression = [NSExpression expressionWithFormat:@"MH_FUNCTION('random', 1, 2, 3, 4, 5)"];
        NSArray *jsonExpression = @[@"random", @1, @2, @3, @4, @5];
        XCTAssertEqualObjects(expression.mgl_jsonExpressionObject, jsonExpression);
        XCTAssertEqualObjects([NSExpression expressionWithMHJSONObject:jsonExpression], expression);
    }
    {
        NSExpression *expression = [NSExpression expressionWithFormat:@"MH_FUNCTION('random', 1, 2, 3, 4)"];
        XCTAssertThrowsSpecificNamed([expression expressionValueWithObject:nil context:nil], NSException, NSInvalidArgumentException);
    }
    {
        NSArray *arguments = @[
            MHConstantExpression(@"one"), MHConstantExpression(@1),
            [NSExpression expressionForVariable:@"one"],
        ];
        NSExpression *nullaryExpression = [NSExpression expressionForFunction:@"MH_LET" arguments:arguments];
        NSExpression *unaryExpression = [NSExpression expressionForFunction:@"MH_LET:" arguments:arguments];
    ...
    }
    {
        NSArray *arguments = @[
            ...
            MHConstantExpression(@YES), MHConstantExpression(@"yes"),
            MHConstantExpression(@NO), MHConstantExpression(@"no"),
        ];
        NSExpression *nullaryExpression = [NSExpression expressionForFunction:@"MH_MATCH" arguments:arguments];
        NSExpression *unaryExpression = [NSExpression expressionForFunction:@"MH_MATCH:" arguments:arguments];
        XCTAssertEqualObjects(nullaryExpression.mgl_jsonExpressionObject, unaryExpression.mgl_jsonExpressionObject);
    }
    {
        NSArray *arguments = @[
            [NSPredicate predicateWithValue:YES],
            MHConstantExpression(@"yes"), MHConstantExpression(@"no"),
        ];
        NSExpression *nullaryExpression = [NSExpression expressionForFunction:@"MH_IF" arguments:arguments];
        NSExpression *unaryExpression = [NSExpression expressionForFunction:@"MH_IF:" arguments:arguments];
        XCTAssertEqualObjects(nullaryExpression.mgl_jsonExpressionObject, unaryExpression.mgl_jsonExpressionObject);
    }
    {
        NSArray *arguments = @[MHConstantExpression(@"zoom")];
        NSExpression *nullaryExpression = [NSExpression expressionForFunction:@"MH_FUNCTION" arguments:arguments];
        NSExpression *unaryExpression = [NSExpression expressionForFunction:@"MH_FUNCTION:" arguments:arguments];
        XCTAssertEqualObjects(nullaryExpression.mgl_jsonExpressionObject, unaryExpression.mgl_jsonExpressionObject);
    }
}
...
- (void)testLocalization {
    {
        NSExpression *original = MHConstantExpression(@"");
        NSExpression *expected = original;
        XCTAssertEqualObjects([original mgl_expressionLocalizedIntoLocale:nil], expected);
    }
    {
        NSExpression *original = MHConstantExpression(@"Old MacDonald");
        NSExpression *expected = original;
        XCTAssertEqualObjects([original mgl_expressionLocalizedIntoLocale:nil], expected);
    }
    {
        NSExpression *original = MHConstantExpression(@"{name_en}");
        NSExpression *expected = original;
        XCTAssertEqualObjects([original mgl_expressionLocalizedIntoLocale:nil], expected);
    }
    {
        NSExpression *keyExpression = [NSExpression expressionForKeyPath:@"name_en"];
        MHAttributedExpression *attributedExpression = [MHAttributedExpression attributedExpression:keyExpression attributes:@{}];
        ...
        MHAttributedExpression *expectedAttributedExpression = [MHAttributedExpression attributedExpression:coalesceExpression attributes:@{}];
    ...
    }
    {
        ...
        NSExpression *expression = [NSExpression expressionWithMHJSONObject:jsonExpression];
...
- (void)testConvenienceInitializers {
    {
        NSExpression *expression = [NSExpression mgl_expressionForConditional:[NSPredicate predicateWithFormat:@"1 = 2"]
                                                               trueExpression:MHConstantExpression(@YES)
                                                             falseExpresssion:MHConstantExpression(@NO)];
    ...
    {
        NSDictionary *values = @{ MHConstantExpression(@1): MHConstantExpression(@"one") };
