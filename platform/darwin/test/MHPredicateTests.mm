...
@interface MHPredicateTests : XCTestCase
@end

@implementation MHPredicateTests
...
- (void)testUnsupportedFilterPredicates {
    ...
    NSPredicate *selectorPredicate = [NSPredicate predicateWithFormat:@"(SELF isKindOfClass: %@)", [MHPolyline class]];
...
- (void)testComparisonPredicates {
    {
        ...
        XCTAssertEqualObjects([NSPredicate predicateWithMHJSONObject:jsonExpression], predicate);
        [self testSymmetryWithPredicate:[NSPredicate predicateWithMHJSONObject:jsonExpression]
                          mustRoundTrip:NO];
    }
    {
    ...
    }
    {
        ...
        XCTAssertEqualObjects([NSPredicate predicateWithMHJSONObject:expected], predicate);
        [self testSymmetryWithPredicate:[NSPredicate predicateWithMHJSONObject:expected]
                          mustRoundTrip:NO];
    }
    {
    ...
    }
    {
        ...
        XCTAssertEqualObjects([NSPredicate predicateWithMHJSONObject:expected], predicateAfter);
        [self testSymmetryWithPredicate:[NSPredicate predicateWithMHJSONObject:expected]
                          mustRoundTrip:YES];
    }
    {
    ...
    }
    {
        ...
        MHPolygon *shape = [MHPolygon polygonWithCoordinates:coordinates count:sizeof(coordinates) / sizeof(coordinates[0])];
    ...
    }
    {
        ...
        MHPolygonFeature *feature = [MHPolygonFeature polygonWithCoordinates:coordinates count:sizeof(coordinates) / sizeof(coordinates[0])];
...
- (void)testComparisonPredicatesWithOptions {
    ...
    {
        NSPredicate *predicate = [NSPredicate predicateWithFormat:@"TRUE = MH_FUNCTION('==', a, 'b', MH_FUNCTION('collator', %@))", @{
            @"case-sensitive": @NO,
