 ...
 */
class MHDocumentationGuideTests: XCTestCase, MHMapViewDelegate {
    var mapView: MHMapView!
    var styleLoadingExpectation: XCTestExpectation!

    override func setUp() {
        super.setUp()
        let styleURL = Bundle(for: MHDocumentationGuideTests.self).url(forResource: "one-liner", withExtension: "json")
        mapView = MHMapView(frame: CGRect(x: 0, y: 0, width: 256, height: 256), styleURL: styleURL)
    ...
    func testMigratingToExpressions$Stops() {
        ...
        var functionExpression = NSExpression(forMHStepping: .zoomLevelVariable,
        ...
        functionExpression = NSExpression(
            forMHStepping: .zoomLevelVariable,
    ...
    func testMigratingToExpressions$Linear() {
        ...
        let symbolSource = MHSource(identifier: "source")
        let symbolLayer = MHSymbolStyleLayer(identifier: "place-city-sm", source: symbolSource)

        let source = MHShapeSource(identifier: "earthquakes", url: url, options: nil)
        ...
        if #available(iOS 15, *) {
            circleExpression = NSExpression(
                forMHInterpolating: NSExpression(forConstantValue: mag),
                curveType: .linear,
                parameters: nil,
                stops: NSExpression(forConstantValue: stops)
            )
    ...
    func testMigratingToExpressions$LinearConvenience() {
        ...
        // #-example-code
        layer.circleColor = NSExpression(forMHInterpolating: NSExpression(forKeyPath: "mag"), curveType: .linear, parameters: nil, stops: NSExpression(forConstantValue: stops))
        // #-end-example-code
    ...
    func testMigratingToExpressions$ExponentialConvenience() {
        ...
        layer.circleRadius = NSExpression(forMHInterpolating: NSExpression.zoomLevelVariable, curveType: MHExpressionInterpolationMode.exponential, parameters: NSExpression(forConstantValue: 1.5), stops: NSExpression(forConstantValue: stops))
        // #-end-example-code
    }
    ...
    func testMigratingToExpressions$Interval() {
        ...
        // #-example-code
            ...
            layer.circleColor = NSExpression(forMHStepping: .zoomLevelVariable,
    ...
    func testMigratingToExpressions$Categorical() {
        ...
        let quarryBlast = NSExpression(forConstantValue: "quarry blast")
            ...
            XCTExpectFailure("Awaiting unit test refactoring for https://github.com/MapHero/MapHero-native/issues/331")
            layer.circleColor = NSExpression(forMHMatchingKey: type,
                                             in: [earthquake: orange, explosion: red, quarryBlast: yellow],
                                             default: defaultColor)
        #endif
    ...
    func testMigratingToExpressions$Cast() {
        ...
        // #-example-code
        let magnitudeLayer = MHSymbolStyleLayer(identifier: "mag-layer", source: source)
