...
/**
 ...
 To add an example:
 1. Add a test case named in the form testMHClass or testMHClass$method.
 ...
 */
class MHDocumentationExampleTests: XCTestCase, MHMapViewDelegate {
    var mapView: MHMapView!
    var styleLoadingExpectation: XCTestExpectation!
    static let styleURL = Bundle(for: MHDocumentationExampleTests.self).url(forResource: "one-liner", withExtension: "json")!

    // Mock MHOfflineStorage singleton so that it doesn't start long-running tasks that could interfere with other tests.
    fileprivate class MHOfflineStorageMock {
        static let shared = MHOfflineStorageMock()
        func addPack(for _: MHOfflineRegion, withContext _: Data, completionHandler: MHOfflinePackAdditionCompletionHandler? = nil) {
            XCTAssert(MHOfflineStorage.shared.responds(to: #selector(MHOfflineStorage.shared.addPack(for:withContext:completionHandler:))))
            if let completionHandler {
                completionHandler(nil, NSError(domain: "MHDocumentationExampleError", code: 0, userInfo: [NSLocalizedDescriptionKey: "\(#function) is mocked and not functional."]))
            }
        }
    }
    ...
    override func setUp() {
        super.setUp()
        mapView = MHMapView(frame: CGRect(x: 0, y: 0, width: 256, height: 256), styleURL: MHDocumentationExampleTests.styleURL)
    ...
    func testMHTilePyramidOfflineRegion() {
        class MHStyle {
            static var lightStyleURL: URL {
                MHDocumentationExampleTests.styleURL
            }
        }

        typealias MHOfflineStorage = MHOfflineStorageMock
        ...
        let region = MHTilePyramidOfflineRegion(styleURL: MHStyle.lightStyleURL, bounds: bbox, fromZoomLevel: 11, toZoomLevel: 14)
    ...
    func testMHShapeOfflineRegion() {
        ...
        let triangle = MHPolygon(coordinates: &coordinates, count: UInt(coordinates.count))
        let region = MHShapeOfflineRegion(styleURL: MHStyle.lightStyleURL, shape: triangle, fromZoomLevel: 11, toZoomLevel: 14)
    ...
    func testMHOfflinePack() {
        typealias MHOfflineStorage = MHOfflineStorageMock

        let northeast = CLLocationCoordinate2D(latitude: 40.989329, longitude: -102.062592)
        let southwest = CLLocationCoordinate2D(latitude: 36.986207, longitude: -109.049896)
        let bbox = MHCoordinateBounds(sw: southwest, ne: northeast)
        let region = MHTilePyramidOfflineRegion(styleURL: MHDocumentationExampleTests.styleURL, bounds: bbox, fromZoomLevel: 11, toZoomLevel: 14)
        let context = "Tile Pyramid Region".data(using: .utf8)!
    ...
    func testMHShape$shapeWithData_encoding_error_() {
        let mainBundle = Bundle(for: MHDocumentationExampleTests.self)

        // #-example-code
        let url = mainBundle.url(forResource: "amsterdam", withExtension: "geojson")!
        let data = try! Data(contentsOf: url)
        let feature = try! MHShape(data: data, encoding: String.Encoding.utf8.rawValue) as! MHShapeCollectionFeature
        // #-end-example-code

        XCTAssertNotNil(feature.shapes.first as? MHPolygonFeature)
    }

    func testMHShapeSource() {
        // #-example-code
    ...
    func testMHRasterTileSource() {
        // #-example-code
        let source = MHRasterTileSource(identifier: "clouds", tileURLTemplates: ["https://example.com/raster-tiles/{z}/{x}/{y}.png"], options: [
            .minimumZoomLevel: 9,
            .maximumZoomLevel: 16,
            .tileSize: 512,
            .attributionInfos: [
                MHAttributionInfo(title: NSAttributedString(string: "© Mapbox"), url: URL(string: "https://mapbox.com")),
            ],
        ])
        mapView.style?.addSource(source)
        // #-end-example-code
    ...
    func testMHVectorTileSource() {
        // #-example-code
        let source = MHVectorTileSource(identifier: "pois", tileURLTemplates: ["https://example.com/vector-tiles/{z}/{x}/{y}.mvt"], options: [
            .minimumZoomLevel: 9,
            .maximumZoomLevel: 16,
            .attributionInfos: [
                MHAttributionInfo(title: NSAttributedString(string: "© Mapbox"), url: URL(string: "https://mapbox.com")),
    ...
    func testMHImageSource() {
        // #-example-code
        let coordinates = MHCoordinateQuad(
    ...
    func testMHCircleStyleLayer() {
        let population = MHVectorTileSource(identifier: "population", configurationURL: URL(string: "https://example.com/style.json")!)
        mapView.style?.addSource(population)

        // #-example-code
        let layer = MHCircleStyleLayer(identifier: "circles", source: population)
        ...
        let stops = NSExpression(forConstantValue: [12: 2,
                                                    22: 180])
        layer.circleRadius = NSExpression(forMHInterpolating: .zoomLevelVariable,
    ...
    func testMHLineStyleLayer() {
        let trails = MHVectorTileSource(identifier: "trails", configurationURL: URL(string: "https://example.com/style.json")!)
        mapView.style?.addSource(trails)

        // #-example-code
        let layer = MHLineStyleLayer(identifier: "trails-path", source: trails)
    ...
    func testMHFillStyleLayer() {
        let parks = MHVectorTileSource(identifier: "parks", configurationURL: URL(string: "https://example.com/style.json")!)
        mapView.style?.addSource(parks)

        // #-example-code
        let layer = MHFillStyleLayer(identifier: "parks", source: parks)
    ...
    func testMHFillExtrusionStyleLayer() {
        let buildings = MHVectorTileSource(identifier: "buildings", configurationURL: URL(string: "https://example.com/style.json")!)
        mapView.style?.addSource(buildings)

        // #-example-code
        let layer = MHFillExtrusionStyleLayer(identifier: "buildings", source: buildings)
    ...
    func testMHHeatmapStyleLayer() {
        let earthquakes = MHShapeSource(identifier: "earthquakes", url: URL(string: "https://example.com/earthquakes.json")!, options: [:])
        mapView.style?.addSource(earthquakes)

        // #-example-code
        let layer = MHHeatmapStyleLayer(identifier: "earthquake-heat", source: earthquakes)

        layer.heatmapWeight = NSExpression(forMHInterpolating: .zoomLevelVariable,
                                           curveType: .linear,
                                           parameters: nil,
                                           stops: NSExpression(forConstantValue: [0: 0,
                                                                                  6: 1]))

        layer.heatmapIntensity = NSExpression(forMHInterpolating: .zoomLevelVariable,
                                              curveType: .linear,
                                              parameters: nil,
    ...
    func testMHSymbolStyleLayer() {
        let pois = MHVectorTileSource(identifier: "pois", configurationURL: URL(string: "https://example.com/style.json")!)
        mapView.style?.addSource(pois)

        // #-example-code
        let layer = MHSymbolStyleLayer(identifier: "coffeeshops", source: pois)
    ...
    func testMHRasterStyleLayer() {
        ...
        // #-example-code
        let layer = MHRasterStyleLayer(identifier: "clouds", source: source)
    ...
    func testMHHillshadeStyleLayer() {
        let source = MHRasterDEMSource(identifier: "dem", tileURLTemplates: ["https://example.com/raster-rgb/{z}/{x}/{y}.png"], options: [
            .minimumZoomLevel: 9,
            .maximumZoomLevel: 16,
            .tileSize: 256,
            .attributionInfos: [
        ...
        let canals = MHVectorTileSource(identifier: "canals", configurationURL: URL(string: "https://example.com/style.json")!)
        mapView.style?.addSource(canals)
        let canalShadowLayer = MHLineStyleLayer(identifier: "waterway-river-canal-shadow", source: canals)
        mapView.style?.addLayer(canalShadowLayer)

        // #-example-code
        let layer = MHHillshadeStyleLayer(identifier: "hills", source: source)
    ...
    func testMHVectorStyleLayer$predicate() {
        let terrain = MHVectorTileSource(identifier: "terrain", configurationURL: URL(string: "https://example.com/style.json")!)
        mapView.style?.addSource(terrain)

        // #-example-code
        let layer = MHLineStyleLayer(identifier: "contour", source: terrain)
    ...
    func testMHMapSnapshotter() throws {
        ...
        // #-example-code
        let camera = MHMapCamera(lookingAtCenter: CLLocationCoordinate2D(latitude: 37.7184, longitude: -122.4365), altitude: 100, pitch: 20, heading: 0)

        let options = MHMapSnapshotOptions(styleURL: MHStyle.satelliteStreetsStyleURL, camera: camera, size: CGSize(width: 320, height: 480))
        options.zoomLevel = 10

        let snapshotter = MHMapSnapshotter(options: options)
    ...
    func testMHAttributedExpression() {
        // #-example-code
        #if os(macOS)
        ...
        let attributes: [MHAttributedExpressionKey: NSExpression] = [.fontNamesAttribute: NSExpression(forConstantValue: ["DIN Offc Pro Italic",
    ...
    func testMHShapeSourceOptionClusterProperties() {
        // #-example-code
        ...
        let clusterShapeData = try! JSONSerialization.data(withJSONObject: geoJSON, options: [])
        let shape = try! MHShape(data: clusterShapeData, encoding: String.Encoding.utf8.rawValue)
        let source = MHShapeSource(identifier: "source", shape: shape, options: options)
        mapView.style?.addSource(source)
    }
