...
@interface MHAttributionInfoTests : XCTestCase

@end

@implementation MHAttributionInfoTests
...
- (void)testParsing {
    ...
    NSMutableArray<MHAttributionInfo *> *infos = [NSMutableArray array];
    for (NSUInteger i = 0; i < sizeof(htmlStrings) / sizeof(htmlStrings[0]); i++) {
        NSArray *subinfos = [MHAttributionInfo attributionInfosFromHTMLString:htmlStrings[i]
                                                                      fontSize:0
                                                                     linkColor:nil];
...
- (void)testStyle {
    ...
    CGFloat fontSize = 72;
    MHColor *color = [MHColor redColor];
    NSMutableArray<MHAttributionInfo *> *infos = [NSMutableArray array];
    for (NSUInteger i = 0; i < sizeof(htmlStrings) / sizeof(htmlStrings[0]); i++) {
        NSArray *subinfos = [MHAttributionInfo attributionInfosFromHTMLString:htmlStrings[i]
                                                                      fontSize:72
    ...
    MHColor *linkColor = [infos[0].title attribute:NSForegroundColorAttributeName atIndex:0 effectiveRange:nil];
