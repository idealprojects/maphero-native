...
mbgl::Tileset MHTileSetFromTileURLTemplates(NSArray<NSString *> *tileURLTemplates, NSDictionary<MHTileSourceOption, id> * _Nullable options) {
    ...
    encodingNumber = options[MHVectorTileSourceOptionEncoding];
    if (encodingNumber) {
        if (![encodingNumber isKindOfClass:[NSValue class]]) {
            [NSException raise:NSInvalidArgumentException
                        format:@"MHTileSourceOptionVectorEncoding must be set to an NSValue or NSNumber."];
        }
        MHVectorTileSourceEncoding encoding;
        [encodingNumber getValue:&encoding];
        switch (encoding) {
            case MHVectorTileSourceEncodingMapbox:
                tileSet.vectorEncoding = mbgl::Tileset::VectorEncoding::Mapbox;
                break;
            case MHVectorTileSourceEncodingMLT:
