...
const MHExceptionName MHBundleNotFoundException = @"MHBundleNotFoundException";

@implementation NSBundle (MHAdditions)

+ (instancetype)mgl_frameworkBundle {
    NSBundle *bundle = [self bundleForClass:[MHSettings class]];

#ifndef MH_CUSTOM_COMBINED_BUNDLE
    if (![bundle.infoDictionary[@"CFBundlePackageType"] isEqualToString:@"FMWK"]) {
        ...
        if (bundlePath) {
            ...
            [NSException raise:MHBundleNotFoundException
                        format:@"The MapHero framework bundle could not be found. If using  MapHero iOS as a static framework, make sure that MapHero bundle is copied into the root of the app bundle."];
        }
    }
#endif
...
+ (nullable NSString *)mgl_applicationBundleIdentifier {
    ...
    if (!bundleIdentifier) {
        bundleIdentifier = [NSBundle bundleForClass:[MHSettings class]].bundleIdentifier;
    }
#endif
