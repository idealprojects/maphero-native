...
#import "NSValue+MHStyleAttributeAdditions.h"
...
const MHExpressionInterpolationMode MHExpressionInterpolationModeLinear = @"linear";
const MHExpressionInterpolationMode MHExpressionInterpolationModeExponential = @"exponential";
...
+ (void)installFunctions {
    Class MHAftermarketExpressionInstaller = [self class];
...
#pragma clang diagnostic ignored "-Wundeclared-selector"
    #define INSTALL_METHOD(sel) \
        { \
            Method method = class_getInstanceMethod(MHAftermarketExpressionInstaller, @selector(sel)); \
    ...
    INSTALL_CONTROL_STRUCTURE(MH_LET);
    INSTALL_CONTROL_STRUCTURE(MH_MATCH);
    INSTALL_CONTROL_STRUCTURE(MH_IF);
    INSTALL_CONTROL_STRUCTURE(MH_FUNCTION);
...
- (NSString *)mgl_attributed:(NSArray<MHAttributedExpression *> *)attributedExpressions {
 ...
 */
- (id)MH_LET:(NSString *)firstVariableName, ... {
    [NSException raise:NSInvalidArgumentException
...
/**
 ...
 arguments as an expression according to the MapHero Style Spec’s
 expression language.
 */
- (id)MH_FUNCTION:(id)firstArgument, ... {
    [NSException raise:NSInvalidArgumentException
...
@implementation NSExpression (MHPrivateAdditions)
...
- (mbgl::Value)mgl_constantMBGLValue {
    ...
    } else if ([value isKindOfClass:[MHColor class]]) {
        auto hexString = [(MHColor *)value mgl_colorForPremultipliedValue].stringify();
        return { hexString };
...
@implementation NSObject (MHExpressionAdditions)
...
@implementation NSNull (MHExpressionAdditions)
...
@implementation NSString (MHExpressionAdditions)
...
@implementation NSNumber (MHExpressionAdditions)
...
@implementation MHColor (MHExpressionAdditions)
...
@implementation NSArray (MHExpressionAdditions)
...
@implementation NSDictionary (MHExpressionAdditions)
...
@implementation NSExpression (MHExpressionAdditions)
...
@implementation MHShape (MHExpressionAdditions)
...
+ (instancetype)mgl_expressionForInterpolatingExpression:(nonnull NSExpression *)inputExpression withCurveType:(nonnull MHExpressionInterpolationMode)curveType parameters:(nullable NSExpression *)parameters stops:(nonnull NSExpression *)stops {
...
static NSDictionary<NSString *, NSString *> *MHExpressionOperatorsByFunctionNames;

NSArray *MHSubexpressionsWithJSONObjects(NSArray *objects) {
    NSMutableArray *subexpressions = [NSMutableArray arrayWithCapacity:objects.count];
    for (id object in objects) {
        NSExpression *expression = [NSExpression expressionWithMHJSONObject:object];
        [subexpressions addObject:expression];
    }
    return subexpressions;
}

+ (instancetype)expressionWithMHJSONObject:(id)object {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        MHFunctionNamesByExpressionOperator = @{
            @"+": @"add:to:",
            @"-": @"from:subtract:",
            @"*": @"multiply:by:",
            @"/": @"divide:by:",
    ...
    if ([object isKindOfClass:[NSString class]] ||
        ...
        [object isKindOfClass:[MHColor class]] ||
        [object isKindOfClass:[MHShape class]]) {
    ...
    if ([object isKindOfClass:[NSDictionary class]]) {
        if (object[@"type"]) {
            ...
            MHShape *shape;
            if (shapeData && !error) {
                shape = [MHShape shapeWithData:shapeData encoding:NSUTF8StringEncoding error:&error];
            }
            if (shape && !error) {
        ...
        [object enumerateKeysAndObjectsUsingBlock:^(id _Nonnull key, id _Nonnull obj, BOOL * _Nonnull stop) {
            dictionary[key] = [NSExpression expressionWithMHJSONObject:obj];
        }];
    ...
    if ([object isKindOfClass:[NSArray class]]) {
        ...
        NSString *functionName = MHFunctionNamesByExpressionOperator[op];
        if (functionName) {
            ...
            return [NSExpression expressionWithMHJSONObject:argumentObjects.firstObject];
            ...
            if (argumentObjects.count == 2) {
                NSExpression *operand = [NSExpression expressionWithMHJSONObject:argumentObjects.lastObject];
                ...
                NSExpression *key = [NSExpression expressionWithMHJSONObject:argumentObjects.firstObject];
            ...
            MHExpressionInterpolationMode interpolationMode = MHExpressionInterpolationModeLinear;
            id curveParameters;
            if ([curveType isEqual:@"exponential"]) {
                curveParameters = interpolationOptions[1];
                interpolationMode = MHExpressionInterpolationModeExponential;
                ...
                interpolationMode = MHExpressionInterpolationModeCubicBezier;
            }
            else {
                curveParameters = [NSNull null];
            }
            NSExpression *curveParameterExpression = [NSExpression expressionWithMHJSONObject:curveParameters];
            argumentObjects = [argumentObjects subarrayWithRange:NSMakeRange(1, argumentObjects.count - 1)];
            NSExpression *inputExpression = [NSExpression expressionWithMHJSONObject:argumentObjects.firstObject];
            ...
            while (NSNumber *key = stopEnumerator.nextObject) {
                ...
                stops[key] = [NSExpression expressionWithMHJSONObject:valueExpression];
            }
            ...
            NSExpression *inputExpression = [NSExpression expressionWithMHJSONObject:argumentObjects[0]];
            NSArray *stopExpressions = [argumentObjects subarrayWithRange:NSMakeRange(1, argumentObjects.count - 1)];
            NSExpression *minimum;
            if (stopExpressions.count % 2) {
                minimum = [NSExpression expressionWithMHJSONObject:stopExpressions.firstObject];
            ...
            while (NSNumber *key = stopEnumerator.nextObject) {
                ...
                if (minimum) {
                    ...
                    minimum = [NSExpression expressionWithMHJSONObject:valueExpression];
                }
            }
            ...
            for (NSUInteger index = 0; index < argumentObjects.count; index++) {
                if (index % 2 == 0 && index != argumentObjects.count - 1) {
                    NSPredicate *predicate = [NSPredicate predicateWithMHJSONObject:argumentObjects[index]];
                    NSExpression *argument = [NSExpression expressionForConstantValue:predicate];
                    [arguments addObject:argument];
                } else {
                    [arguments addObject:[NSExpression expressionWithMHJSONObject:argumentObjects[index]]];
                }
            }
            ...
            for (NSUInteger index = 0; index < argumentObjects.count; index++) {
                NSExpression *option = [NSExpression expressionWithMHJSONObject:argumentObjects[index]];
                ...
                if (index > 0 && index < argumentObjects.count - 1 && !(index % 2 == 0) && [argumentObjects[index] isKindOfClass:[NSArray class]]) {
                    option = [NSExpression expressionForAggregate:MHSubexpressionsWithJSONObjects(argumentObjects[index])];
                }
            ...
            for (NSUInteger index = 0; index < argumentObjects.count; index+=2) {
                NSExpression *expression = [NSExpression expressionWithMHJSONObject:argumentObjects[index]];
                ...
                for (NSString *key in attrs.allKeys) {
                    attrs[key] = [NSExpression expressionWithMHJSONObject:attrs[key]];
                }
                MHAttributedExpression *attributedExpression = [[MHAttributedExpression alloc] initWithExpression:expression attributes:attrs];
            ...
            for (id operand in argumentObjects) {
                [expressions addObject:[NSExpression expressionWithMHJSONObject:operand]];
            }
...
- (id)mgl_jsonExpressionObject {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        MHExpressionOperatorsByFunctionNames = @{
            @"add:to:": @"+",
            @"from:subtract:": @"-",
            @"multiply:by:": @"*",
            @"divide:by:": @"/",
    ...
    switch (self.expressionType) {
        ...
        case NSConstantValueExpressionType: {
            ...
            if ([constantValue isKindOfClass:[MHColor class]]) {
            ...
            if ([constantValue isKindOfClass:[NSValue class]]) {
                ...
                if (strcmp([boxedValue objCType], @encode(MHEdgeInsets)) == 0) {
                    // padding [x, y]
                    std::array<float, 4> mglValue = boxedValue.mgl_paddingArrayValue;
                    return @[@"literal", @[@(mglValue[0]), @(mglValue[1]), @(mglValue[2]), @(mglValue[3])]];
                }
            }
            if ([constantValue isKindOfClass:[MHAttributedExpression class]]) {
                MHAttributedExpression *attributedExpression = (MHAttributedExpression *)constantValue;
                id jsonObject = attributedExpression.expression.mgl_jsonExpressionObject;
                NSMutableDictionary<MHAttributedExpressionKey, NSExpression *> *attributedDictionary = [NSMutableDictionary dictionary];
            ...
            if ([constantValue isKindOfClass:[MHShape class]]) {
                MHShape *shape = (MHShape *)constantValue;
        ...
        case NSFunctionExpressionType: {
            ...
            NSString *op = MHExpressionOperatorsByFunctionNames[function];
            if (op) {
                ...
                return expressionObject;
            } else if ([function isEqualToString:@"MH_IF"] ||
                       [function isEqualToString:@"MH_IF:"] ||
                       [function isEqualToString:@"mgl_if:"]) {
                return self.mgl_jsonIfExpressionObject;
            } else if ([function isEqualToString:@"MH_MATCH"] ||
                       [function isEqualToString:@"MH_MATCH:"] ||
                ...
                if ([type isEqualToString:@"NSString"]) {
...
#if TARGET_OS_IPHONE
                else if ([type isEqualToString:@"UIColor"] || [type isEqualToString:@"MHColor"]) {
                    return @[@"to-color", object];
                }
#else
                else if ([type isEqualToString:@"NSColor"] || [type isEqualToString:@"MHColor"]) {
                    return @[@"to-color", object];
                }
#endif
                    ...
                    if (([operand expressionType] != NSConstantValueExpressionType) ||
                        ([operand expressionType] == NSConstantValueExpressionType &&
                         [[operand constantValue] isKindOfClass:[MHColor class]])) {
            ...
            } else if ([function isEqualToString:@"MH_FUNCTION"] ||
                       [function isEqualToString:@"MH_FUNCTION:"]) {
            ...
            } else if (op == (NSString *) [MHColor class] && [function isEqualToString:@"colorWithRed:green:blue:alpha:"]) {
                NSArray *arguments = self.arguments.mgl_jsonExpressionObject;
...
- (id)mgl_jsonMatchExpressionObject {
    BOOL isAftermarketFunction = [self.function hasPrefix:@"MH_MATCH"];
    NSUInteger minimumIndex = isAftermarketFunction ? 1 : 0;
...
- (id)mgl_jsonIfExpressionObject {
    BOOL isAftermarketFunction = [self.function hasPrefix:@"MH_IF"];
    NSUInteger minimumIndex = isAftermarketFunction ? 1 : 0;
 ...
 */
NSArray<NSExpression *> *MHLocalizedCollection(NSArray<NSExpression *> *collection, NSLocale * _Nullable locale) {
...
/**
 ...
 dictionary.
 */
NSDictionary<NSNumber *, NSExpression *> *MHLocalizedStopDictionary(NSDictionary<NSNumber *, NSExpression *> *stops, NSLocale * _Nullable locale) {
    ...
    switch (self.expressionType) {
        case NSConstantValueExpressionType: {
            if ([self.constantValue isKindOfClass:[NSDictionary class]]) {
                NSDictionary *localizedStops = MHLocalizedStopDictionary(self.constantValue, locale);
                ...
                NSArray *localizedValues = MHLocalizedCollection(self.constantValue, locale);
                if (localizedValues != self.constantValue) {
                    return [NSExpression expressionForConstantValue:localizedValues];
                }
            } else if ([self.constantValue isKindOfClass:[MHAttributedExpression class]]) {
                MHAttributedExpression *attributedExpression = (MHAttributedExpression *)self.constantValue;
                NSExpression *localizedExpression = [attributedExpression.expression mgl_expressionLocalizedIntoLocale:locale];
                MHAttributedExpression *localizedAttributedExpression = [MHAttributedExpression attributedExpression:localizedExpression attributes:attributedExpression.attributes];
        ...
        case NSKeyPathExpressionType: {
            if ([self.keyPath isEqualToString:@"name"] || [self.keyPath hasPrefix:@"name_"]) {
                ...
                if (![locale.localeIdentifier isEqualToString:@"mul"]) {
                    ...
                    NSString *preferredLanguage = [MHVectorTileSource preferredMapboxStreetsLanguageForPreferences:preferences];
        ...
        case NSFunctionExpressionType: {
            ...
            NSArray *localizedArguments = MHLocalizedCollection(arguments, locale);
        ...
        case NSAggregateExpressionType: {
            ...
            if ([collection isKindOfClass:[NSArray class]]) {
                NSArray *localizedCollection = MHLocalizedCollection(collection, locale);
