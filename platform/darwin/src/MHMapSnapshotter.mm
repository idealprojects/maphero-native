...
NSString * const MHSnapshotterAnnotationSpritePrefix = @"org.maplibre.sprites.";
...
@interface MHMapSnapshotter() <MHMultiPointDelegate>
...
@implementation MHMapSnapshotter {
    ...
    NSMutableArray<id<MHAnnotation>> *_annotationsToInstall;
...
- (void)installAnnotations:(NSArray<id<MHAnnotation>> *)annotations
{
    MHLogDebug(@"install: %lu annotations", annotations.count);
    ...
    for (id <MHAnnotation> annotation in annotations)
    {
        ...
        if ([annotation isKindOfClass:[MHMultiPoint class]])
        {
            ...
            if ( ! symbolName)
            {
                symbolName = [MHSnapshotterAnnotationSpritePrefix stringByAppendingString:annotationImage.reuseIdentifier];
...
- (void)configureWithOptions:(MHMapSnapshotOptions *)options {
    ...
    // (Was a ternary of std::string vs nullptr — constructing std::string(nullptr)
    // on the nil branch, which newer clang rejects under -Wnonnull.)
    std::optional<std::string> localFontFamilyName;
    if (config.localFontFamilyName) {
        localFontFamilyName = std::string(config.localFontFamilyName.UTF8String);
    }
...
- (mbgl::Color)strokeColorForShapeAnnotation:(MHShape *)annotation
{
    if ([self.delegate respondsToSelector:@selector(mapSnapshotter:strokeColorForShapeAnnotation:)]) {
        MHColor *color = [self.delegate mapSnapshotter:self strokeColorForShapeAnnotation:annotation];
...
- (mbgl::Color)fillColorForPolygonAnnotation:(MHPolygon *)annotation
{
    if ([self.delegate respondsToSelector:@selector(mapSnapshotter:fillColorForPolygonAnnotation:)]){
        MHColor *color = [self.delegate mapSnapshotter:self fillColorForPolygonAnnotation:annotation];
        return color.mgl_color;
    }
    return mbgl::Color::white();
}
