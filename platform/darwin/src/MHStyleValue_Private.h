...
NS_INLINE MHTransition MHTransitionFromOptions(const mbgl::style::TransitionOptions &options) {
  MHTransition transition;
  transition.duration =
      MHTimeIntervalFromDuration(options.duration.value_or(mbgl::Duration::zero()));
  transition.delay = MHTimeIntervalFromDuration(options.delay.value_or(mbgl::Duration::zero()));

  return transition;
}

NS_INLINE mbgl::style::TransitionOptions MHOptionsFromTransition(MHTransition transition) {
  mbgl::style::TransitionOptions options{{MHDurationFromTimeInterval(transition.duration)},
                                         {MHDurationFromTimeInterval(transition.delay)}};
  return options;
}

std::unique_ptr<mbgl::style::expression::Expression> MHClusterPropertyFromNSExpression(
...
class MHStyleValueTransformer {
 public:
  ...
  NSExpression *toExpression(const mbgl::style::ColorRampPropertyValue &mbglValue) {
    ...
    return [NSExpression
        expressionWithMHJSONObject:MHJSONObjectFromMBGLExpression(mbglValue.getExpression())];
  }
 ...
 private:  // Private utilities for converting from mgl to mbgl values
  ...
  template <typename MBGLEnum = MBGLType,
            ...
            typename MHEnum = ObjCEnum,
            class = typename std::enable_if<!std::is_enum<MHEnum>::value>::type>
  NSObject *toRawStyleSpecValue(NSObject *rawMHValue, MBGLEnum &) {
    if ([rawMHValue isKindOfClass:[NSValue class]]) {
      const auto rawNSValue = (NSValue *)rawMHValue;
  ...
  NSString *toRawStyleSpecValue(ObjCType rawValue, MBGLEnum &) {
    ...
    return @(mbgl::Enum<MHEnum>::toString(mglEnum));
  }

  NSObject *toRawStyleSpecValue(MHColor *color, MBGLType &) {
  ...
  // Color
  void getMBGLValue(MHColor *rawValue, mbgl::Color &mbglValue) { mbglValue = rawValue.mgl_color; }
  ...
  void getMBGLValue(id rawValue, mbgl::VariableAnchorOffsetCollection &mbglValue) {
    if ([rawValue isKindOfClass:[NSArray class]]) {
      ...
      for (NSUInteger i = 0; i < array.count; i += 2) {
        ...
        getMBGLValue<mbgl::style::SymbolAnchorType, MHTextAnchor>(array[i], anchor);
  ...
  // Enumerations
  template <typename MBGLEnum = MBGLType, typename MHEnum = ObjCEnum,
            class = typename std::enable_if<std::is_enum<MBGLEnum>::value>::type,
            class = typename std::enable_if<std::is_enum<MHEnum>::value>::type>
  void getMBGLValue(id rawValue, MBGLEnum &mbglValue) {
    if ([rawValue isKindOfClass:[NSString class]]) {
      ...
      auto str = mbgl::Enum<MHEnum>::toString(mglEnum);
 ...
 private:  // Private utilities for converting from mbgl to mgl values
  // Bool
  static NSNumber *toMHRawStyleValue(const bool mbglStopValue) { return @(mbglStopValue); }

  // Float
  static NSNumber *toMHRawStyleValue(const float mbglStopValue) { return @(mbglStopValue); }

  // Integer
  static NSNumber *toMHRawStyleValue(const int64_t mbglStopValue) { return @(mbglStopValue); }

  // String
  static NSString *toMHRawStyleValue(const std::string &mbglStopValue) {
    return @(mbglStopValue.c_str());
  }

  // Formatted
  static NSString *toMHRawStyleValue(const mbgl::style::expression::Formatted &mbglStopValue) {
    return @(mbglStopValue.toString().c_str());
  }

  // Offsets
  static NSValue *toMHRawStyleValue(const std::array<float, 2> &mbglStopValue) {
    return [NSValue mgl_valueWithOffsetArray:mbglStopValue];
  }

  // Padding as array<float, 4>
  static NSValue *toMHRawStyleValue(const std::array<float, 4> &mbglStopValue) {
    return [NSValue mgl_valueWithPaddingArray:mbglStopValue];
  }

  // Padding type
  static NSValue *toMHRawStyleValue(const mbgl::Padding &mbglStopValue) {
    return [NSValue mgl_valueWithPaddingArray:mbglStopValue.toArray()];
  }

  // Color
  static MHColor *toMHRawStyleValue(const mbgl::Color mbglStopValue) {
    return [MHColor mgl_colorWithColor:mbglStopValue];
  }

  // VariableAnchorOffsetCollection
  static NSArray<NSExpression *> *toMHRawStyleValue(
    ...
    for (const auto &anchorOffset : mbglStopValue) {
      NSString *anchor = toMHRawStyleValue(anchorOffset.anchorType);
  ...
  // Image
  static NSString *toMHRawStyleValue(const mbgl::style::expression::Image &mbglImageValue) {
  ...
  static NSArray<NSExpression *> *toMHRawStyleValue(
    ...
    for (const auto &mbglElement : mbglStopValue) {
      [array addObject:[NSExpression expressionForConstantValue:toMHRawStyleValue(mbglElement)]];
    }
    return array;
  }

  static NSValue *toMHRawStyleValue(const mbgl::style::Position &mbglStopValue) {
    std::array<float, 3> spherical = mbglStopValue.getSpherical();
    MHSphericalPosition position =
        MHSphericalPositionMake(spherical[0], spherical[1], spherical[2]);
    return [NSValue valueWithMHSphericalPosition:position];
  }

  // Enumerations
  template <typename MBGLEnum = MBGLType, typename MHEnum = ObjCEnum>
  static NSString *toMHRawStyleValue(const MBGLEnum &value) {
  ...
  class PropertyExpressionEvaluator {
   public:
    ...
    NSExpression *operator()(const MBGLType &value) const {
      id constantValue = toMHRawStyleValue(value);
