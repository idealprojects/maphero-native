...
const MHShapeSourceOption MHShapeSourceOptionClusterMinPoints = @"MHShapeSourceOptionClusterMinPoints";
...
mbgl::Immutable<mbgl::style::GeoJSONOptions> MHGeoJSONOptionsFromDictionary(NSDictionary<MHShapeSourceOption, id> *options) {
    ...
    if (NSNumber *value = options[MHShapeSourceOptionClusterMinPoints]) {
        if (![value isKindOfClass:[NSNumber class]]) {
            [NSException raise:NSInvalidArgumentException
                        format:@"MHShapeSourceOptionClusterMinPoints must be an NSNumber."];
        }
