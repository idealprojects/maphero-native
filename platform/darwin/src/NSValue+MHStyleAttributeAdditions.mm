...
@implementation NSValue (MHStyleAttributeAdditions)
...
+ (instancetype)mgl_valueWithPaddingArray:(std::array<float, 4>)paddingArray
{
    ...
    return [NSValue value:&insets withObjCType:@encode(MHEdgeInsets)];
}

- (std::array<float, 2>)mgl_offsetArrayValue
{
    MHAssert(strcmp(self.objCType, @encode(CGVector)) == 0, @"Value does not represent a CGVector");
    CGVector vector;
...
{
    MHAssert(strcmp(self.objCType, @encode(MHEdgeInsets)) == 0, @"Value does not represent an NSEdgeInsets/UIEdgeInsets");
    MHEdgeInsets insets;
...
{
    MHAssert(strcmp(self.objCType, @encode(MHSphericalPosition)) == 0, @"Value does not represent an MHSphericalPosition");
