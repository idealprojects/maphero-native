...
@protocol MHLocationManagerDelegate;

/**
 The ``MHLocationManager`` protocol defines a set of methods that a class must
 implement in order to serve as the location manager of an ``MHMapView``. A location
 ...
 To receive location updates from an object that conforms to the ``MHLocationManager``
 protocol, use the optional methods available in the ``MHLocationManagerDelegate`` protocol.
 */
@protocol MHLocationManager <NSObject>
...
/**
 ...
 The default value of this property is `kCLDistanceFilterNone` when ``MHMapView`` uses its
...
/**
 ...
 The default value is `kCLLocationAccuracyBest` when ``MHMapView`` uses its
 default location manager.
...
/**
 ...
 ``MHLocationManager/desiredAccuracy`` property to any value. If the value is
 `CLAccuracyAuthorizationReducedAccuracy`, setting ``MHLocationManager/desiredAccuracy`` to a value
...
/**
 ...
 The default value is `CLActivityTypeOther` when ``MHMapView`` uses its
 default location manager.
...
/**
 ...
 Do not set the location manager’s delegate yourself. ``MHMapView`` sets this property
 after the location manager becomes ``MHMapView``’s location manager.
 */
@property (nonatomic, weak) id<MHLocationManagerDelegate> delegate;
...
/**
 The ``MHLocationManagerDelegate`` protocol defines a set of methods that respond
 to location updates from an ``MHLocationManager`` object that is serving as the
 location manager of an ``MHMapView``.
 */
...
/**
 ...
 contains multiple `CLLocation` objects when ``MHMapView`` uses  its
 default location manager.
 */
- (void)locationManager:(id<MHLocationManager>)manager
 ...
 */
- (void)locationManager:(id<MHLocationManager>)manager didUpdateHeading:(CLHeading *)newHeading;
 ...
 */
- (void)locationManager:(id<MHLocationManager>)manager didFailWithError:(nonnull NSError *)error;
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 140000
/**
