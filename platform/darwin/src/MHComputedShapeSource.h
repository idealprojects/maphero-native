...
/**
 ...
 an ``MHComputedShapeSource`` should be wrapped to accomodate coordinates with
 longitudes beyond −180 and 180. The default value is `NO`.

 Setting this option to `YES` affects rendering performance.

 This option is used with the ``MHComputedShapeSource`` class; it is ignored when
 creating an ``MHShapeSource`` object.
 */
FOUNDATION_EXTERN MH_EXPORT const MHShapeSourceOption MHShapeSourceOptionWrapsCoordinates;

/**
 An `NSNumber` object containing a Boolean value; specifies whether the shape of
 an ``MHComputedShapeSource`` should be clipped at the edge of each tile. The
 default value is `NO`.

 Setting this option to `YES` affects rendering performance. Use this option to
 clip ``MHPolyline``s and ``MHPolyline``s at tile boundaries without artifacts.

 This option is used with the ``MHComputedShapeSource`` class; it is ignored when
 creating an ``MHShapeSource`` object.
 */
FOUNDATION_EXTERN MH_EXPORT const MHShapeSourceOption MHShapeSourceOptionClipsCoordinates;

FOUNDATION_EXTERN MH_EXPORT MHExceptionName const MHInvalidDatasourceException;

/**
 Data source for ``MHComputedShapeSource``. This protocol defines two optional methods for fetching
 ...
 */
@protocol MHComputedShapeSourceDataSource <NSObject>
 ...
 */
- (NSArray<MHShape<MHFeature> *> *)featuresInTileAtX:(NSUInteger)x
                                                     y:(NSUInteger)y
...
/**
 ...
 @param zoomLevel Tile zoom level.
 */
- (NSArray<MHShape<MHFeature> *> *)featuresInCoordinateBounds:(MHCoordinateBounds)bounds
                                                      zoomLevel:(NSUInteger)zoomLevel;

@end

/**
 ``MHComputedShapeSource`` is a map content source that supplies vector shapes,
 one tile at a time, to be shown on the map on demand. You implement a class
 conforming to the ``MHComputedShapeSourceDataSource`` protocol that returns
 instances of ``MHShape`` or ``MHShape``, then add a computed shape source to an
 ...
 ``MHComputedShapeSource`` is similar to ``MHComputedShapeSource`` but is optimized for
 ...
 <a href="http://geojson.org/">GeoJSON</a> data, use the ``MHShapeSource`` class.
 For static tiles or Mapbox Vector Tiles, use the ``MHVectorTileSource`` class.

 You can add and remove sources dynamically using methods such as
 ``MHStyle/addSource:`` and ``MHStyle/sourceWithIdentifier:``. This class
 cannot be represented in a style JSON file; you must add it ot the style at
 runtime.
 */
MH_EXPORT
@interface MHComputedShapeSource : MHSource
...
/**
 ...
 ``MHShapeSourceOptionBuffer``,
 ``MHShapeSourceOptionSimplificationTolerance``,
 ``MHShapeSourceOptionWrapsCoordinates``, and
 ``MHShapeSourceOptionClipsCoordinates``. Shapes provided by a computed
...
/**
 ...
 ``MHShapeSourceOptionSimplificationTolerance``,
 ``MHShapeSourceOptionWrapsCoordinates``, and
 ``MHShapeSourceOptionClipsCoordinates``. Shapes provided by a computed shape
...
/**
 ...
 will immediately be invoked on the ``MHComputedShapeSourceDataSource``.
 @param x Tile X coordinate.
 @param y Tile Y coordinate.
 @param zoomLevel Tile zoom level.
 */
...
/**
 ...
 through ``MHComputedShapeSourceDataSource/``
 @param features  Features for the tile.
 @param x         Tile X coordinate.
 @param y         Tile Y coordinate.
 @param zoomLevel Tile zoom level.
 */
- (void)setFeatures:(NSArray<MHShape<MHFeature> *> *)features
          inTileAtX:(NSUInteger)x
                  y:(NSUInteger)y
          zoomLevel:(NSUInteger)zoomLevel;

/**
 An object that implements the ``MHComputedShapeSourceDataSource`` protocol that will be queried
 for tile data.
 */
