...
/**
 ``MHVectorStyleLayer`` is an abstract superclass for style layers whose content
 is defined by an ``MHShapeSource`` or ``MHShapeSource`` object.

 Create instances of ``MHCircleStyleLayer``, ``MHCircleStyleLayer``,
 ``MHFillExtrusionStyleLayer``, ``MHFillExtrusionStyleLayer``, ``MHFillExtrusionStyleLayer``, and
 ``MHSymbolStyleLayer`` in order to use ``MHSymbolStyleLayer``'s properties and
 methods. Do not create instances of ``MHVectorStyleLayer`` directly, and do not
 create your own subclasses of this class.
 */
MH_EXPORT
@interface MHVectorStyleLayer : MHForegroundStyleLayer
