...
@implementation MHAttributionInfo

+ (NSArray<MHAttributionInfo *> *)attributionInfosFromHTMLString:(nullable NSString *)htmlString fontSize:(CGFloat)fontSize linkColor:(nullable MHColor *)linkColor {
    if (!htmlString) {
        return @[];
    }
    ...
    ^(id _Nullable value, NSRange range, BOOL * _Nonnull stop) {
        MHCAssert(!value || [value isKindOfClass:[NSURL class]], @"If present, URL attribute must be an NSURL.");
        ...
        MHAttributionInfo *info = [[MHAttributionInfo alloc] initWithTitle:unlinkedTitle URL:value];
...
+ (NSAttributedString *)attributedStringForAttributionInfos:(NSArray<MHAttributionInfo *> *)attributionInfos {
...
{
    MHAttributionInfo *info = [[[self class] allocWithZone:zone] initWithTitle:_title
                                                                            URL:_URL];
...
- (NSAttributedString *)titleWithStyle:(MHAttributionInfoStyle)style
{
    ...
    BOOL isAbbreviated = (style == MHAttributionInfoStyleShort);
 ...
 */
- (NSComparisonResult)subsetCompare:(MHAttributionInfo *)otherInfo {
...
@implementation NSMutableArray (MHAttributionInfoAdditions)

- (void)growArrayByAddingAttributionInfo:(MHAttributionInfo *)info {
    __block BOOL didInsertInfo = NO;
    __block BOOL shouldAddInfo = YES;
    [self enumerateObjectsUsingBlock:^(MHAttributionInfo * _Nonnull existingInfo, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)growArrayByAddingAttributionInfosFromArray:(NSArray<MHAttributionInfo *> *)infos {
    for (MHAttributionInfo *info in infos) {
