...
@implementation NSPredicate (MHPrivateAdditions)
...
+ (instancetype)mgl_predicateWithFilter:(mbgl::style::Filter)filter
{
    if (filter.expression) {
        id jsonObject = MHJSONObjectFromMBGLExpression(**filter.expression);
        return [NSPredicate predicateWithMHJSONObject:jsonObject];
    } else {
        return nil;
    }
}
...
@implementation NSPredicate (MHAdditions)

NSArray *MHSubpredicatesWithJSONObjects(NSArray *objects) {
    NSMutableArray *subpredicates = [NSMutableArray arrayWithCapacity:objects.count];
    for (id object in objects) {
        NSPredicate *predicate = [NSPredicate predicateWithMHJSONObject:object];
        [subpredicates addObject:predicate];
    }
    return subpredicates;
}

static NSDictionary * const MHPredicateOperatorTypesByJSONOperator = @{
...
+ (instancetype)predicateWithMHJSONObject:(id)object {
    ...
    MHAssert([object isKindOfClass:[NSArray class]], @"Condition for case expression should be an expression.");
    NSArray *objects = (NSArray *)object;
    NSString *op = objects.firstObject;
    
    NSNumber *operatorTypeNumber = MHPredicateOperatorTypesByJSONOperator[op];
    if (operatorTypeNumber) {
        ...
        if (objects.count > 3) {
            ...
            MHCAssert([collatorExpression isKindOfClass:[NSArray class]], @"Collators must be dictionaries.");
            MHCAssert(collatorExpression.count == 2, @"Malformed collator expression");
            NSDictionary *collator = collatorExpression[1];
            MHCAssert([collator isKindOfClass:[NSDictionary class]], @"Malformed collator in collator expression");
        ...
        NSArray *subexpressions = MHSubexpressionsWithJSONObjects([objects subarrayWithRange:NSMakeRange(1, objects.count - 1)]);
    ...
    if ([op isEqualToString:@"!"]) {
        NSArray *subpredicates = MHSubpredicatesWithJSONObjects([objects subarrayWithRange:NSMakeRange(1, objects.count - 1)]);
    ...
    if ([op isEqualToString:@"all"]) {
        NSArray<NSPredicate *> *subpredicates = MHSubpredicatesWithJSONObjects([objects subarrayWithRange:NSMakeRange(1, objects.count - 1)]);
...
@implementation NSPredicate (MHExpressionAdditions)
