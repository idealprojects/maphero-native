...
/**
 ...
 ``MHVectorTileSourceEncoding``.

 The default value for this option is
 ``MHVectorTileSourceEncoding/MHVectorTileSourceEncodingMapbox``.
 */
FOUNDATION_EXTERN MH_EXPORT const MHTileSourceOption MHVectorTileSourceOptionEncoding;

/**
 The encoding formula used to generate the raster-dem tileset
*/

typedef NS_ENUM(NSUInteger, MHVectorTileSourceEncoding) {

  /**
     Vector tiles in [MVT format](https://github.com/mapbox/vector-tile-spec).
  */
  MHVectorTileSourceEncodingMapbox = 0,

  /**
   Vector tiles in [MLT format](https://github.com/maplibre/maplibre-tile-spec).
  */
  MHVectorTileSourceEncodingMLT = 1,
};

/**
 ``MHVectorTileSource`` is a map content source that supplies tiled vector data
 ...
 A vector tile source is added to an ``MHStyle`` object along with one or more
 ``MHVectorStyleLayer`` objects. A vector style layer defines the appearance of
 any content supplied by the vector tile source.

 ``MHVectorTileSource`` is optimized for data sets that are too large to fit
 ...
 <a href="http://geojson.org/">GeoJSON</a> data, use the ``MHShapeSource``
 class. For tiled data that changes dynamically, the ``MHComputedShapeSource``
 ...
 ``MHVectorTileSource`` object that you can use to initialize new style layers.
 ...
 ```swift
 let source = MHVectorTileSource(identifier: "pois", tileURLTemplates:
 ...
 an ``MHVectorTileSource`` object.
 */
MH_EXPORT
@interface MHVectorTileSource : MHTileSource
...
/**
 ...
 source loads a feature if the source is added to an ``MHMapView``’s style; that
 ...
 */
- (NSArray<id<MHFeature>> *)featuresInSourceLayersWithIdentifiers:
