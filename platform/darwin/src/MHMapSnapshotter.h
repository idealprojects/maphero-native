...
/**
 ...
 To access this object, use ``MHMapSnapshotter/startWithOverlayHandler:completionHandler:``.
 */

MH_EXPORT
@interface MHMapSnapshotOverlay : NSObject
...
draw custom overlays rendered with Core Graphics.
 ...
 @param snapshotOverlay The ``MHMapSnapshotOverlay`` provided during snapshot drawing.
 */
typedef void (^MHMapSnapshotOverlayHandler)(MHMapSnapshotOverlay *snapshotOverlay);

/**
 The options to use when creating images with the ``MHMapSnapshotter``.
 */
MH_EXPORT
@interface MHMapSnapshotOptions : NSObject <NSCopying>
 ...
 */
@property (nonatomic) MHMapCamera *camera;
 ...
 */
@property (nonatomic) MHCoordinateBounds coordinateBounds;
 ...
 */
MH_EXPORT
@interface MHMapSnapshot : NSObject
...
/**
 ...
 @param snapshot The ``MHMapSnapshot`` that was generated or `nil` if an error
    occurred.
 @param error The error that occured or `nil` when successful.
 */
typedef void (^MHMapSnapshotCompletionHandler)(MHMapSnapshot *_Nullable snapshot,
                                                NSError *_Nullable error);

/**
 An ``MHMapSnapshotter`` generates static raster images of the map. Each snapshot
 image depicts a portion of a map defined by an ``MHMapSnapshotOptions`` object
 you provide. The snapshotter generates an ``MHMapSnapshot`` object
 asynchronously, calling ``MHMapSnapshotterDelegate`` methods if defined, then
 ...
 For an interactive map, use the ``MHMapView`` class. Both ``MHMapSnapshotter``
 and ``MHMapView`` are compatible with offline packs managed by the
 ``MHOfflineStorage`` class.
 ...
 ```swift
 let camera = MHMapCamera(lookingAtCenter: CLLocationCoordinate2D(latitude: 37.7184, longitude:
 -122.4365), altitude: 100, pitch: 20, heading: 0)

 let options = MHMapSnapshotOptions(styleURL: MHStyle.satelliteStreetsStyleURL, camera: camera,
 ...
 ``MHMapSnapshotter`` to generate a static image based on an ``MHMapSnapshotter``
 object's style, camera, and view bounds.
 */
MH_EXPORT
@interface MHMapSnapshotter : NSObject <MHStylable>
 ...
 */
- (void)startWithCompletionHandler:(MHMapSnapshotCompletionHandler)completionHandler;
...
- (void)startWithQueue:(dispatch_queue_t)queue
     completionHandler:(MHMapSnapshotCompletionHandler)completionHandler;
...
/**
 ...
 top of the resulting ``MHMapSnapshot``.
 ...
 @param completionHandler The block to call with a finished snapshot. The block
     is executed on the main queue.
 */
- (void)startWithOverlayHandler:(MHMapSnapshotOverlayHandler)overlayHandler
              completionHandler:(MHMapSnapshotCompletionHandler)completionHandler;
...
/**
 ...
 the snapshot, create a new ``MHMapSnapshotter`` object.
 */
- (void)cancel;
 ...
 */
@property (nonatomic) MHMapSnapshotOptions *options;
 ...
 */
@property (nonatomic, weak) id<MHMapSnapshotterDelegate> delegate;

/**
 The style displayed in the resulting snapshot.

 Unlike the ``MHMapSnapshotOptions/styleURL`` property, this property is set to
 ...
 ``MHMapSnapshotterDelegate/mapSnapshotter:didFinishLoadingStyle:`` method. It
...
/**
 ...
 ``MHMapSnapshotter`` object.
 */
@protocol MHMapSnapshotterDelegate <NSObject>
@optional
...
*/
- (void)mapSnapshotterDidFail:(MHMapSnapshotter *)snapshotter withError:(NSError *)error;

/**
 Tells the delegate that the snapshotter has just finished loading a style.

 This method is called in response to
 ``MHMapSnapshotter/startWithQueue:completionHandler:`` as long as the
 ``MHMapSnapshotter/delegate`` property is set. Changes to sources or layers of
