...
MH_EXPORT
/**
 ``MHImageSource`` is a content source that is used for a georeferenced raster
 ...
 use the ``MHStyle/setImage:forName:`` method. To configure a point
 annotation’s image, use the ``MHAnnotationImage`` class.

 The geographic location of the raster image content, supplied with
 ``MHCoordinateQuad``, can be non-axis aligned.
 ``MHImageSource`` supports raster content from `NSURL`, `NSImage` (macOS), or
 `UIImage` (iOS).
 An image source is added to an ``MHStyle`` object along with one or more
 ...
 ``MHImageSource`` object that you can use to initialize new style layers. You
 ...
 let source = MHImageSource(identifier: "radar", coordinateQuad: coordinates, url: URL(string:
 "https://maplibre.org/maplibre-gl-js-docs/assets/radar.gif")!) mapView.style?.addSource(source)
 ```

 #### Related examples
 TODO: Add an image, learn how to add an image to your map using ``MHImageSource``.
 */
MH_EXPORT
@interface MHImageSource : MHSource
...
- (instancetype)initWithIdentifier:(NSString *)identifier
                    coordinateQuad:(MHCoordinateQuad)coordinateQuad
                               URL:(NSURL *)url;
 ...
 */
@property (nonatomic, retain, nullable) MHImage *image;

/**
 The coordinates at which the corners of the source image will be placed.
 */
@property (nonatomic) MHCoordinateQuad coordinates;
