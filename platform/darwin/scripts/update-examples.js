...
const examplesSrc = fs.readFileSync('platform/darwin/test/MHDocumentationExampleTests.swift', 'utf8');

// Regex extracts the following block
// /** Front matter to describe the example. **/
// func testMHClass$member() {
//     ...
//     // #-example-code
...
// 1 (test method name): "MHClass" or "MHClass$member" or "MHClass$initWithArg_anotherArg_"
// 2 (indentation): "    "
  ...
  _.forEach(docJson, function (result) {
    _.forEach(result, function (structure, path) {
      ...
      _.forEachRight(structure['key.substructure'], function completeSubstructure(substructure, idx, substructures, symbolPath) {
        ...
        if (!example) {
          console.error(`MHDocumentationExampleTests.test${testMethodName}() not found.`);
          process.exit(1);
        }
