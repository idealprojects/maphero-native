...
MapHero Native integrates [Tracy profiler](https://github.com/wolfpld/tracy) which offers an easy way to understand and optimize your application's CPU and GPU performance.
...
Instrumentation is generally the first step in profiling applications that use MapHero. Once slow inner-loop routines are identified, additional hardware vendor specific tools can be used to collect hardware counters and optimize low level CPU and GPU code.
...
#### Enabling instrumentation in MapHero Native
...
OpenGL is also supported in MapHero native. Tracy support is currently missing for other APIs such as Metal and need to be added separately.
...
The above macros can be added inside MapHero code and also in the application code that calls MapHero.

The following macros should only be used if there are changes to MapHero internals:
...
Make sure you generate the MapHero project with the option `MH_USE_TRACY` enabled.

As an example, the glfw sample is used.

With CMake, in MapHero repository root do
~~~bash
# generate project
cmake -B build -GNinja -DCMAKE_C_COMPILER=clang -DCMAKE_CXX_COMPILER=clang++ -DCMAKE_BUILD_TYPE=RelWithDebInfo -DMH_WITH_CLANG_TIDY=OFF -DMH_WITH_COVERAGE=OFF -DCMAKE_BUILD_WITH_INSTALL_RPATH=ON -DMH_USE_TRACY=ON
# build
...
#### Connecting the profiler to a MapHero Android application
