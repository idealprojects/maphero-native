...
Before we jump into the coordinate system of MapHero Native, let's
...
will project a WGS 84 point to a map tile rendered in MapHero
Native.
...
*World* to denote the world MapHero Native renders. The word *world*
...
MapHero Native renders map tiles in a range of zoom levels on a 3D
...
MapHero Native requires a series of coordinate transformations to render a
...
1.  Design and define model in the *Local Space*. We call anything that
    ...
    its own coordinate system. For MapHero Native, in local space
...
Going forward, scoping our discussion only to MapHero Native, we
...
EPSG:3857 is used by MapHero Native as a default coordinate system
...
tile, we need to know what is the *extent* of the tile. MapHero
Native follows Mapbox Vector Tile (MVT) spec. Following said spec,
MapHero Native internally normalizes each tile to an *extent* of
...
only one object. And in MapHero Native, this plane has an origin of (0,0),
positioned on the top left.

## Device Coordinates

World space for MapHero Native contains a plane in 3D with all the
tiles for any specific zoom level. Map tiles are hierarchical in nature.
As in they have different zoom levels. MapHero Native internally
...
*Projection Matrix*. In MapHero Native, a camera initialization
...
matrix. And MapHero Native uses a *perspective projection*.
...
shaders that MapHero Native at the time of writing uses 4
...
On top of this, MapHero Native uses a field of view of 36.87 degrees
...
the tile coordinates we wish to render in MapHero Native but only in
...
[^11]: There are other coordinate systems such as EPSG:54001 that uses
    ...
    document focuses on EPSG:3857 because MapHero Native uses it by
    default.
...
[^15]: The piece of code we run on GPU is called a shader. We will see
    more how shaders influence MapHero Native rendering later in the
    document.
